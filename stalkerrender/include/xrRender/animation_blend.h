...
IC	bool CBlend::update_time			( float dt )
{
	...
	float	const END_EPS1	=	SAMPLE_SPF+XrMath::EPS;
	bool	at_end			=	running_fwrd && ( timeCurrent > ( timeTotal-END_EPS1 ) );
	...
	if( !stop_at_end )
	{
		...
		if( at_end )
			timeCurrent -= ( timeTotal-END_EPS1 );
	...
	if( at_end )
	{
		timeCurrent	= timeTotal-END_EPS1;		// stop@end - time frozen at the end
