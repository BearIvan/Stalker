...
//			dfObject		= (1<<1),
	...
	// execute
		void				ExecuteAnimate		(PAPI::Particle *particles, bsize p_cnt, float dt);
        void				ExecuteCollision	(PAPI::Particle *particles, bsize p_cnt, float dt, CParticleEffect* owner, CollisionCallback cb);
	public:
                            CPEDef				();
                            ~CPEDef				();
