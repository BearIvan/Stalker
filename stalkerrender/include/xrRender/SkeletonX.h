...
protected:
	...
	void					_Render_soft		(ref_geom& hGeom, 	bsize vCount,	bsize iOffset, bsize pCount);
	void					_Render				(ref_geom& hGeom, 	bsize vCount,	bsize iOffset, bsize pCount);
	void					_Load				(const char* N,		IReader *data, bsize& dwVertCount);

	virtual void			_Load_hw			(Fvisual& V,		void *data)			= 0;
	virtual void			_CollectBoneFaces	(Fvisual* V, bsize iBase, bsize iCount)		= 0;
	...
	virtual void			_FillVertices		(const Fmatrix& view, CSkeletonWallmark& wm, const Fvector& normal, float size, Fvisual* V, u16 bone_id, bsize iBase, bsize iCount)			=0;
	...
	virtual BOOL			_PickBone			(IKinematics::pick_result &r, float range, const Fvector& S, const Fvector& D, Fvisual* V, u16 bone_id, bsize iBase, bsize iCount)						=0;
public:
