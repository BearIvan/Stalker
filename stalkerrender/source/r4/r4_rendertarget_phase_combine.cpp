...
void	CRenderTarget::phase_combine	()
{
	...
	if (!_menu_pp)
	{
		...
		Fvector4	ambclr			= { XrMath::max(envdesc.ambient.x*2,minamb),	XrMath::max(envdesc.ambient.y*2,minamb),			XrMath::max(envdesc.ambient.z*2,minamb),	0	};
...
//.		Fvector4	envclr			= { envdesc.sky_color.x*2+XrMath::EPS,	envdesc.sky_color.y*2+XrMath::EPS,	envdesc.sky_color.z*2+XrMath::EPS,	envdesc.weight					};
		Fvector4	envclr			= { envdesc.hemi_color.x*2+XrMath::EPS,	envdesc.hemi_color.y*2+XrMath::EPS,	envdesc.hemi_color.z*2+XrMath::EPS,	envdesc.weight					};
   ...
   // for msaa we need a resolved color buffer - Holger
	...
	BOOL	bDistort	= RImplementation.o.distortion_enabled;				// This can be modified
	{
		...
		if (bDistort)		
		{
			...
			FLOAT ColorRGBA1[4] = { 127.0f/255.0f, 127.0f/255.0f, 0.0f, 127.0f/255.0f};
			if( !RImplementation.o.dx10_msaa )
			{
				u_setrt(rt_Generic_1,0,0,HW.pBaseZB);		// Now RT is a distortion mask
				HW.pContext->ClearRenderTargetView( rt_Generic_1->pRT, ColorRGBA1);
			}
			else
			{
				u_setrt(rt_Generic_1_r,0,0,RImplementation.Target->rt_MSAADepth->pZRT);		// Now RT is a distortion mask
				HW.pContext->ClearRenderTargetView( rt_Generic_1_r->pRT, ColorRGBA1);
			}
			RCache.set_CullMode			(CULL_CCW);
			RCache.set_Stencil			(FALSE);
			RCache.set_ColorWriteEnable	();
...
//.		RCache.set_c				("dof_params",	ps_r2_dof.x, ps_r2_dof.y, ps_r2_dof.z, ps_r2_dof_sky);
	...
	if (1) for (u32 it=0; it<dbg_planes.size(); it++)
	{
		...
		Fvector				p01,p11,p2,p3;
		float				sz	= 100.f;
		p01.mad				(zero,L_right,sz).mad	(L_dir,sz);
		p11.mad				(zero,L_right,sz).mad	(L_dir,-sz);
		p2.mad				(zero,L_right,-sz).mad	(L_dir,-sz);
		p3.mad				(zero,L_right,-sz).mad	(L_dir,+sz);
		RCache.dbg_DrawTRI	(Fidentity,p01,p11,p2,0xffffffff);
		RCache.dbg_DrawTRI	(Fidentity,p2,p3,p01,0xffffffff);
	}
