...
public:
	...
	void						u_setrt					(bsize W, bsize H, IDirect3DSurface9* _1, IDirect3DSurface9* _2, IDirect3DSurface9* _3, IDirect3DSurface9* zb);
	...
	virtual void				set_duality_h			(float	f)		{ param_duality_h=XrMath::abs(f);			}
	virtual void				set_duality_v			(float	f)		{ param_duality_v=XrMath::abs(f);			}
	virtual void				set_noise				(float	f)		{ param_noise=f;					}
	virtual void				set_noise_scale			(float	f)		{ param_noise_scale=f;				}
	virtual void				set_noise_fps			(float	f)		{ param_noise_fps=XrMath::abs(f)+XrMath::EPS_S;	}
