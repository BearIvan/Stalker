...
CRenderTarget::CRenderTarget		()
{
	...
	//	NORMAL
	{
		...
		if (RImplementation.o.advancedpp&&gameVersionController->getGame()!=GameVersionController::SOC)
	...
	if (RImplementation.o.HW_smap)
	{
		...
		else 	if (gameVersionController->getGame() == GameVersionController::CS)
		{
			if (RImplementation.o.advancedpp)
				s_accum_direct_volumetric.create("accum_volumetric_sun");
		}
	...
	// Build textures
	{
		...
		else
		{
			...
			{
				string_path					name;
				BearCore::BearString::Printf(name, "%s%d", r2_jitter, it);
