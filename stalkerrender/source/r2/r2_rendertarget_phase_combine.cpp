...
void	CRenderTarget::phase_combine	()
{
	...
	// sun-params
		...
		RCache.Vertex.Unlock(4, g_combine_VP->vb_stride);
		dxEnvDescriptorMixerRender *envdescren = 0;
		if (GameVersionController::SOC != gameVersionController->getGame())
		{
			CEnvDescriptorMixer& envdesc = *ENV.CurrentEnv;
			envdescren = (dxEnvDescriptorMixerRender*)(&*envdesc.m_pDescriptorMixer);
	...
	if (GameVersionController::SOC != gameVersionController->getGame())
	{
		/* if (!RImplementation.o.fp16_blend)*/	ENV.RenderFlares();	// lens-flares
	}
	else
	{
		ENV_SOC.RenderFlares();
	}
