...
void CRenderTarget::accum_direct		(u32 sub_phase)
{
	...
	// Perform lighting
	{
		...
		{
			fBias = (SE_SUN_NEAR == sub_phase) ? ps_r2_sun_depth_near_bias : ps_r2_sun_depth_far_bias;
		}
		else
		{
			fBias = (SE_SUN_NEAR == sub_phase) ? (-ps_r2_sun_depth_near_bias) : ps_r2_sun_depth_far_bias;
		...
		Fmatrix				m_clouds_shadow;
		{
			...
			if (gameVersionController->getGame() != GameVersionController::SOC)
			{
				w_dir = ENV.CurrentEnv->wind_direction;
			}
			else
			{
				w_dir = ENV_SOC.CurrentEnv.wind_direction;
			}
		...
		if (SE_SUN_NEAR==sub_phase)	{
			...
			if (gameVersionController->getGame() == GameVersionController::SOC)
			{
				extern float	OLES_SUN_LIMIT_SOC;
				zMax = OLES_SUN_LIMIT_SOC;
			}
			else
			{
...
void CRenderTarget::accum_direct_volumetric	(u32 sub_phase, const u32 Offset, const Fmatrix &mShadow)
{
...

	{
		if (GameVersionController::SOC != gameVersionController->getGame())
		{
			...
			CEnvDescriptor&	E = *ENV.CurrentEnv;
