...
void CLight_Compute_XFORM_and_VIS::compute_xf_spot	(light* L)
{
	...
	if (L->right.square_magnitude()>XrMath::EPS)				{
	...
	// Note: we XrMath::clamp screen space area to ONE, although it is not correct at all
	float	dist				= Device.vCameraPosition.distance_to(L->spatial.sphere.P)-L->spatial.sphere.R;
			if (dist<0)	dist	= 0;
	float	ssa					= XrMath::clampr	(L->range*L->range / (1.f+dist*dist),0.f,1.f);
	...
	float	widefactor			= L->cone/XrMath::deg2rad(90.f);	// 
	...
	// final size calc
	u32 _size					= XrMath::iFloor( factor * SMAP_adapt_optimal );
	if (_size<SMAP_adapt_min)	_size	= SMAP_adapt_min;
	if (_size>SMAP_adapt_max)	_size	= SMAP_adapt_max;
	int _epsilon				= XrMath::iCeil	(float(_size)*0.01f);
	int _diff					= XrMath::abs	(int(_size)-int(_cached_size));
	...
	// XrMath::min(L->cone + XrMath::deg2rad(4.5f), PI*0.98f) - Here, it is needed to enlarge the shadow map frustum to include also 
	...
	{
		L->X.S.project.build_projection(L->cone + XrMath::deg2rad(3.5f), 1.f, SMAP_near_plane, L->range + XrMath::EPS_S);
	}
	else
	{
