	...
	if (ps_r2_ls_flags.test(R2FLAG_ZFILL))		{
...

		{
			m_project.build_projection(
				...
				z_distance * ENV.CurrentEnv->far_plane);
		}
		else
		{
			m_project.build_projection(
				XrMath::deg2rad(Device.fFOV/* *Device.fASPECT*/),
				Device.fASPECT, VIEWPORT_NEAR,
				z_distance * ENV_SOC.CurrentEnv.far_plane);
		}
...
/*		if (0)
	...
	if (bSUN)	{
		...
		if (!ps_r2_ls_flags_ext.is(R2FLAGEXT_SUN_OLD) && gameVersionController->getGame() != GameVersionController::SOC)
		{
			render_sun_cascades();
		}
		else
