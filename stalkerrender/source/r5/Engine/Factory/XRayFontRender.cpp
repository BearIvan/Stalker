...
XRayFontRender::XRayFontRender() /*: m_index_count(0), m_vertex_count(0)*/
{
	...
	HW->FontRenders.push_back(this);
}

XRayFontRender::~XRayFontRender()
{
	auto item = bear_find_if(HW->FontRenders.begin(), HW->FontRenders.end(), [&](XRayFontRender* right)->bool {return right == this; });
	BEAR_ASSERT(item != HW->FontRenders.end());
	HW->FontRenders.erase(item);
}
