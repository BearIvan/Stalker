#include "pch.h"
int XRayRenderInterface::TranslateSector(IRender_Sector* pSector)
{
	if (!pSector)
		return -1;
...
IRender_Sector* XRayRenderInterface::detectSector(const Fvector& P)
{
	...
	int ID;
	if (id1 >= 0) {
		if (id2 >= 0) ID = (range1 <= range2 + XrMath::EPS) ? id1 : id2;	// both was found
	...
	if (ID == id1) {
		...
		CPortal* pPortal = (CPortal*)m_Portals[pTri->dummy];
