...
static LPCSTR name_blend_type(CBlend::ECurvature blend)
{
	static xr_token token_blend[] =
	{
		...
		{"eFORCEDWORD"		,static_cast<int>(CBlend::eFORCEDWORD)}
	};
...
void	XRayKinematicsAnimated::LL_BoneMatrixBuild(CBoneInstance& bi, const Fmatrix* parent, const SKeyTable& keys)
{
	...
	VERIFY(dbg_box.contains(bi.mTransform.c));//, (make_string("model: %s has strange bone position, matrix : ", getDebugName().c_str()) + XrDumpString::get_string(bi.mTransform)).c_str());
