...
wm_shader dxWallMarkArray::GenerateWallmark()
{
	...
	if (!m_CollideMarks.empty())
		((dxUIShader*)&*res)->hShader = m_CollideMarks[static_cast<bsize>(::Random.randI(0,static_cast<s32>(m_CollideMarks.size())))];
	return res;
}
...
ref_shader*	dxWallMarkArray::dxGenerateWallmark()
{
	return m_CollideMarks.empty()?NULL:
		&m_CollideMarks[static_cast<bsize>(::Random.randI(0, static_cast<s32>(m_CollideMarks.size())))];
}