...
class cl_fog_plane	: public R_constant_setup {
	...
	virtual void setup(R_constant* C)
	{
		if (marker!=Device.dwFrame)
		{
			...
			{
				float A = ENV_SOC.CurrentEnv.fog_near;
				float B = 1 / (ENV_SOC.CurrentEnv.fog_far - A);
				result.set(-plane.x*B, -plane.y*B, -plane.z*B, 1 - (plane.w - A)*B);								// view-plane
			}
			else
			{
				float A = ENV.CurrentEnv->fog_near;
				float B = 1 / (ENV.CurrentEnv->fog_far - A);
...
class cl_fog_params	: public R_constant_setup {
	...
	virtual void setup(R_constant* C)
	{
		if (marker!=Device.dwFrame)
		{
			...
			if (gameVersionController->getGame() == gameVersionController->SOC)
			{
				float	n = ENV_SOC.CurrentEnv.fog_near;
				float	f = ENV_SOC.CurrentEnv.fog_far;
				float	r = 1 / (f - n);
				result.set(-n*r, r, r, r);
			}
			else
			{
				float	n =ENV.CurrentEnv->fog_near;
				float	f = ENV.CurrentEnv->fog_far;
				float	r = 1 / (f - n);
				result.set(-n*r, r, r, r);
			}
...
class cl_times		: public R_constant_setup {
	virtual void setup(R_constant* C)
	{
		...
		RCache.set_c	(C,t,t*10,t/10,XrMath::sin(t))	;
	}
};
...
class cl_amb_color	: public R_constant_setup {
	...
	virtual void setup	(R_constant* C)	{
		if (marker!=Device.dwFrame)	{
			if (gameVersionController->getGame() == gameVersionController->SOC)
			{
				CEnvDescriptorMixerSOC&	desc = ENV_SOC.CurrentEnv;
				result.set(desc.ambient.x, desc.ambient.y, desc.ambient.z, desc.weight);
			}
			else
			{
				CEnvDescriptorMixer&	desc =  *ENV.CurrentEnv;
				result.set(desc.ambient.x, desc.ambient.y, desc.ambient.z, desc.weight);
			}
