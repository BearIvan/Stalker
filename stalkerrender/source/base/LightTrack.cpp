...
#endif
#if RENDER==R_R1
#define ps_r2_dhemi_scale 1.f
#else
extern float		ps_r2_dhemi_scale;
#endif
//////////////////////////////////////////////////////////////////////
...
const float		hdir_soc		[lt_hemisamples][3] = 
{
...
const float		hdir_cop		[lt_hemisamples][3] = 
{
	{-0.26287,	0.52573,	0.80902	},
	{0.27639,	0.44721,	0.85065	},
	{-0.95106,	0.00000,	0.30902	},
...
void	CROS_impl::update(IRenderable* O)
{
	...
	if (gameVersionController->getGame() == GameVersionController::SOC)
	{
		...
		if (MODE & IRender_ObjectSpecific::TRACE_SUN) {
			if (--result_sun < 0) {
				...
				Fvector	direction1;	direction1.set(sun->direction).invert().normalize();
				sun_value = !(g_pGameLevel->ObjectSpace.RayTest(position, direction1, 500.f, collide::rqtBoth, &cache_sun, _object)) ? 1.f : 0.f;
			}
		}
		...
		if (MODE & IRender_ObjectSpecific::TRACE_HEMI) {
			for (u32 it = 0; it < (u32)ps_r2_dhemi_count; it++) {	// five samples per one frame
				...
				// take sample
				Fvector	direction1;	direction1.set(hdir_soc[sample][0], hdir_soc[sample][1], hdir_soc[sample][2]).normalize();
				//.			result[sample]	=	!g_pGameLevel->ObjectSpace.RayTest(position,direction,50.f,collide::rqtBoth,&cache[sample],_object);
				result[sample] = !g_pGameLevel->ObjectSpace.RayTest(position, direction1, 50.f, collide::rqtStatic, &cache[sample], _object);
	...
	//	float	l_i				=	1.f-l_f;
		...
		CEnvDescriptorSOC&	desc = ENV_SOC.CurrentEnv;
	...
	else
	{
		...
		Fvector			accum; Fvector			hemi; Fvector			sun_;
		if (gameVersionController->getGame() == gameVersionController->SOC)
		{
			CEnvDescriptorSOC&	desc = ENV_SOC.CurrentEnv;
			accum = { desc.ambient.x,		desc.ambient.y,		desc.ambient.z };
			hemi = { desc.hemi_color.x,	desc.hemi_color.y,	desc.hemi_color.z };
			sun_ = { desc.sun_color.x,		desc.sun_color.y,	desc.sun_color.z };
		}
		else
		{
...
void CROS_impl::calc_sky_hemi_value(Fvector& position, CObject* _object)
{
	...
	if	(MODE & IRender_ObjectSpecific::TRACE_HEMI)	
	{
		...
		for (u32 it=0; it<(u32)ps_r2_dhemi_count;	it++)		{	// five samples per one frame
			...
			// take sample
			Fvector	direction;	direction.set	(hdir_cop[sample][0], hdir_cop[sample][1], hdir_cop[sample][2]).normalize	();
			//.			result[sample]	=	!g_pGameLevel->ObjectSpace.RayTest(position,direction,50.f,collide::rqtBoth,&cache[sample],_object);
	...
	for (int it=0; it<result_count; it++)
	{
		if (result[it])
		{
			accum_hemi(hemi_cube, Fvector3().set(hdir_cop[it][0], hdir_cop[it][1], hdir_cop[it][2]), ps_r2_dhemi_sky_scale);
		}
	}
}
