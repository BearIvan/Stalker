...
void CBackend::dbg_DrawOBB		(Fmatrix& T1, Fvector& half_dim, u32 C1)
{
	Fmatrix mL2W_Transform,mScaleTransform;

	mScaleTransform.scale(half_dim);
	mL2W_Transform.mul_43(T1,mScaleTransform);

	FVF::L  aabb[8];
	aabb[0].set( -1, -1, -1, C1 ); // 0
	aabb[1].set( -1, +1, -1, C1 ); // 1
	aabb[2].set( +1, +1, -1, C1 ); // 2
	aabb[3].set( +1, -1, -1, C1 ); // 3
	aabb[4].set( -1, -1, +1, C1 ); // 4
	aabb[5].set( -1, +1, +1, C1 ); // 5
	aabb[6].set( +1, +1, +1, C1 ); // 6
	aabb[7].set( +1, -1, +1, C1 ); // 7
...
void CBackend::dbg_DrawTRI	(Fmatrix& T1, Fvector& p1, Fvector& p2, Fvector& p3, u32 C1)
{
	FVF::L	tri[3];
	tri[0].p = p1; tri[0].color = C1;
	tri[1].p = p2; tri[1].color = C1;
	tri[2].p = p3; tri[2].color = C1;

	set_xform_world	(T1);
	dbg_Draw(D3DPT_TRIANGLESTRIP,tri,1);
}
void CBackend::dbg_DrawLINE(Fmatrix& T1, Fvector& p1, Fvector& p2, u32 C1)
{
	FVF::L	line[2];
	line[0].p = p1; line[0].color = C1;
	line[1].p = p2; line[1].color = C1;

	set_xform_world	(T1);
	dbg_Draw(D3DPT_LINELIST,line,1);
}
void CBackend::dbg_DrawEllipse(Fmatrix& T1, u32 C1)
{
	float gVertices[] =
	{
	...
	for (int i=0; i<vcnt; i++) {
		int k=i*3;
		verts[i].set(gVertices[k],gVertices[k+1],gVertices[k+2],C1);
	}
