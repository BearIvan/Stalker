...
{
	float	eps					=	XrMath::EPS_L;	//XrMath::max	(range*0.001f,XrMath::EPS_L);
...
void	light::set_range		(float R)			{
	float	eps					=	XrMath::max	(range*0.1f,XrMath::EPS_L);
	if (XrMath::fsimilar(range,R,eps))	return	;
	range						= R		;
	spatial_move				();
};

void	light::set_cone			(float angle)		{
	if (XrMath::fsimilar(cone,angle))	return	;
	VERIFY						(cone < XrMath::deg2rad(121.f));	// 120 is hard limit for lights
	cone						= angle;
	spatial_move				();
}
void	light::set_rotation		(const Fvector& D, const Fvector& R)	{ 
	...
	right.normalize(R);
	if (!XrMath::fsimilar(1.f, old_D.dotproduct(D)))	spatial_move	();
}
	...
	case IRender_Light::SPOT		:	
		{
			...
			VERIFY2						(cone < XrMath::deg2rad(121.f), "Too large light-cone angle. Maybe you have passed it in 'degrees'?");
			if (cone>=XrMath::PI_DIV_2)			{
				// obtused-angled
				spatial.sphere.P.mad	(position,direction,range);
				spatial.sphere.R		= range * tanf(cone/2.f);
			} else {
				// acute-angled
				spatial.sphere.R		= range / (2.f * XrMath::sqr(XrMath::cos(cone/2.f)));
...
void	light::xform_calc			()
{
	...
	if (_valid(l_dir_m) && l_dir_m>XrMath::EPS_S)	L_dir.div(l_dir_m);
	else									L_dir.set(0,0,1);

	// R&N
	if (right.square_magnitude()>XrMath::EPS)				{
...
float	light::get_LOD					()
{
	...
	float	distSQ			= Device.vCameraPosition.distance_to_sqr(spatial.sphere.P)+XrMath::EPS;
	float	ssa				= ps_r2_slight_fade * spatial.sphere.R/distSQ;
	float	lod				= XrMath::sqrt(XrMath::clampr((ssa - r_ssaGLOD_end)/(r_ssaGLOD_start-r_ssaGLOD_end),0.f,1.f));
	return	lod	;
}
