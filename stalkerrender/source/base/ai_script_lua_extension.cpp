...
int __cdecl Lua::LuaOut(Lua::ELuaMessageType tLuaMessageType, LPCSTR caFormat, ...)
{
    ...
    xr_strcpy(S2, SS);
    S1 = S2 + xr_strlen(SS);
	BearCore::BearString::PrintfVa(S1, 4096 - xr_strlen(S), caFormat, l_tMarker);
...
bool bfDoFile(CLuaVirtualMachine* tpLuaVM, const bchar*fsPath,LPCSTR caScriptName, LPCSTR caNameSpaceName, bool bCall)
{
...
bool Script::bfLoadFileIntoNamespace(CLuaVirtualMachine* tpLuaVM, const bchar*fsPath, LPCSTR caScriptName, LPCSTR caNamespaceName, bool bCall)
{
