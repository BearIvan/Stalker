...
extern float OLES_SUN_LIMIT_SOC;	//	actually sun_far
extern float OLES_SUN_LIMIT_COP;	//	actually sun_far
...
void		xrRender_initconsole	()
{
	...
	{
		ps_r__WallmarkTTL = 300.f;
		ps_r__LOD = 1.f;
		ps_r__tf_Anisotropic = 4;
		ps_r1_dlights_clip = 30.f;
		ps_r2_ssaLOD_A = 48.f;
		ps_r2_ssaLOD_B = 32.f;
		ps_r2_ls_flags = { R2FLAG_SUN | R2FLAG_SUN_IGNORE_PORTALS | R2FLAG_EXP_DONT_TEST_UNSHADOWED | R2FLAG_USE_NVSTENCIL | R2FLAG_EXP_SPLIT_SCENE | R2FLAG_EXP_MT_CALC };
		ps_r2_tonemap_middlegray = 0.25f;			// r2-only
		ps_r2_tonemap_adaptation = 5.f;				// r2-only
		ps_r2_tonemap_low_lum = 0.001f;			// r2-only
		ps_r2_tonemap_amount = 0.5f;				// r2-only
		ps_r2_ls_bloom_kernel_g = 3.3f;				// r2-only
		ps_r2_ls_bloom_speed = 10.f;				// r2-only
		ps_r2_ls_bloom_kernel_scale = 1.0f;				// r2-only	// gauss
		ps_r2_ls_bloom_threshold = .3f;				// r2-only
		ps_r2_mblur = .5f;				// .5f
		ps_r2_ls_depth_bias = -0.0001f;			// -0.0001f
		ps_r2_sun_tsm_projection = 0.18f;			// 0.18f
		ps_r2_sun_tsm_bias = -0.05f;			// 
		ps_r2_sun_near = 12.f;				// 12.0f
		ps_r2_sun_near_border = 0.66f;			// 1.0f
		ps_r2_sun_depth_far_bias = 0.00000f;			// -0.0000f
		ps_r2_sun_depth_near_scale = 1.00001f;			// 1.00001f
		ps_r2_sun_depth_near_bias = -0.00004f;		// -0.00005f
		ps_r2_gmaterial = 0.f;				// 
		ps_r2_zfill = 0.1f;				// .1f
...
//	CMD3(CCC_Mask,		"r2_exp_donttest_uns",	&ps_r2_ls_flags,			R2FLAG_EXP_DONT_TEST_UNSHADOWED);
	...
	if (gameVersionController->getGame() == GameVersionController::SOC)
	{
		CMD4(CCC_Float, "r2_sun_far", &OLES_SUN_LIMIT_SOC, 51.f, 180.f);
	}
	else
	{
		CMD4(CCC_Float, "r2_sun_far", &OLES_SUN_LIMIT_COP, 51.f, 180.f);
	}
