...
#include "engine/xr_efflensflare.h"
...
void dxEnvDescriptorRender::OnDeviceCreate(EnvDescriptorRef *owner)
{
	if (xr_strlen(owner->getSkyName()))
		sky_texture.create		(owner->getSkyName());
	if (xr_strlen(owner->getEvnSkyName()))
		sky_texture_env.create(owner->getEvnSkyName());
	if (xr_strlen(owner->getCloudsName()))
		clouds_texture.create(owner->getCloudsName());
}
...
void dxEnvironmentRender::OnFrame(EnvironmentRef *env)
{
	dxEnvDescriptorMixerRender *mixRen = 0;
	if (gameVersionController->getGame() == gameVersionController->SOC)
		mixRen = (dxEnvDescriptorMixerRender*)&*((*(CEnvironmentSOC*)(env)).CurrentEnv.m_pDescriptorMixer);
	else
		mixRen = (dxEnvDescriptorMixerRender*)&*((*(CEnvironment*)(env)).CurrentEnv->m_pDescriptorMixer);
	...
	Fvector3	fog_color;
	if (gameVersionController->getGame() == gameVersionController->SOC)
		fog_color = (*dynamic_cast<CEnvironmentSOC*>(env)).CurrentEnv.fog_color;
	else
		fog_color = (*dynamic_cast<CEnvironment*>(env)).CurrentEnv->fog_color;
	...
	CHK_DX(HW.pDevice->SetRenderState(D3DRS_FOGCOLOR,XrColor::color_rgba_f(fog_color.x, fog_color.y, fog_color.z, 0)));
		if (gameVersionController->getGame() == gameVersionController->SOC)
		{
			CHK_DX(HW.pDevice->SetRenderState(D3DRS_FOGSTART, *(u32 *)(&(*dynamic_cast<CEnvironmentSOC*>(env)).CurrentEnv.fog_near)));
			CHK_DX(HW.pDevice->SetRenderState(D3DRS_FOGEND, *(u32 *)(&(*dynamic_cast<CEnvironmentSOC*>(env)).CurrentEnv.fog_far)));
		}
		else
		{
			CHK_DX(HW.pDevice->SetRenderState(D3DRS_FOGSTART, *(u32 *)(&(*dynamic_cast<CEnvironment*>(env)).CurrentEnv->fog_near)));
			CHK_DX(HW.pDevice->SetRenderState(D3DRS_FOGEND, *(u32 *)(&(*dynamic_cast<CEnvironment*>(env)).CurrentEnv->fog_far)));
		}
#endif	//	USE_DX10
}
...
void dxEnvironmentRender::RenderSky(EnvironmentRef *env_)
{
	...
	{
		CEnvironment&env = (*dynamic_cast<CEnvironment*>(env_));
		if (env.bNeed_re_create_env)
		{
			sh_2sky.create(&m_b_skybox, "skybox_2t");
			sh_2geom.create(v_skybox_fvf, RCache.Vertex.Buffer(), RCache.Index.Buffer());
...
void dxEnvironmentRender::RenderClouds(EnvironmentRef *env_)
{
	if (gameVersionController->getGame() == gameVersionController->SOC)
	{
