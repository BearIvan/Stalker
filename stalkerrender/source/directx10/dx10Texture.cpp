...
ID3DBaseTexture*	CRender::texture_load(LPCSTR fRName, bsize& ret_msize, bool bStaging)
{
	...
	//	Staging control
	static bool bAllowStaging = !strstr(GetCommandLine(),"-no_staging");
...
#ifdef USE_DX11
		R_CHK2 (D3DX11GetImageInfoFromMemory(S->pointer(),S->length(), 0, &IMG, 0), fname);
#else
		R_CHK2 (D3DX10GetImageInfoFromMemory(S->pointer(),S->length(), 0, &IMG, 0), fname);
#endif
