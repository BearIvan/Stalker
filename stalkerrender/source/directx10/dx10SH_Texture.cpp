...
void CTexture::Load		()
{
	...
	// Check for OGM
	if (FS.ExistFile(TEXT("%textures%"),*cName,TEXT(".ogm"))){
		// AVI
...
//				_w, _h, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &pTexture, NULL );
		...
		/*if (FS.ExistFile(TEXT("%textures%"), *cName, TEXT(".ogm")){
			...
			if (FS.ExistFile(TEXT("%textures%"), *cName, TEXT(".seq")))
			{
				// Sequence
				string256 buffer;
				IReader* _fs = XRayBearReader::Create(FS.Read(TEXT("%textures%"), *cName, TEXT(".seq")));
				...
				while (!_fs->eof())
				{
					...
					if (buffer[0])	
					{
						...
						if (pSurface)	
						{
							...
							flags.MemoryUsage		+=static_cast<UINT>( mem);
						}
					}
				}
				pSurface	= 0;
