...
void dx103DFluidData::ParseProfile(const xr_string &Profile)
{
	...
	CInifile	ini(TEXT("%config%"), Profile.c_str(),TRUE,TRUE,FALSE);

	Msg("Reading fog volume config: %s", Profile.c_str());
	...
	for ( u32 i=0; i<iEmittersNum; ++i )
	{
		...
		Emitter.m_InvSigma_2 = 1.0f / XrMath::sqr(Emitter.m_InvSigma_2);
