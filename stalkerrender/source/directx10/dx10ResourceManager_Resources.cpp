...
SVS*	CResourceManager::_CreateVS		(LPCSTR _name)
{
	...
	else
	{
		...
		string_path					shName;
		{
			...
			for (ptrdiff_t ia = 0; ia < size; ia++)shName[ia] = _name[ia];
			shName[size] = 0;
		}
		...
		if (!FS.ExistFile(TEXT("%shaders%"), cname))
		{
			string1024			tmp;
			...
			file				= XRayBearReader::Create(FS.Read(TEXT("%shaders%"), cname));
		}
		else
		{
			file =XRayBearReader::Create(FS.Read(TEXT("%shaders%"), cname));
		}
		bsize	const size			= file->length();
		...
		HRESULT	const _hr		= ::Render->shader_compile(name,(DWORD const*)data,static_cast<UINT>(size), c_entry, c_target, D3D10_SHADER_PACK_MATRIX_ROW_MAJOR, (void*&)_vs );
...
SPS*	CResourceManager::_CreatePS			(LPCSTR _name)
{
	...
	else
	{
		...
		for (ptrdiff_t ia = 0; ia < strSize; ia++)shName[ia] = _name[ia];
		shName[strSize] = 0;
		...
		// duplicate and zero-terminate
		IReader*		file = 0;// = FS.r_open(cname);
		//	TODO: DX10: HACK: Implement all shaders. Remove this for PS
		if (!FS.ExistFile(TEXT("%shaders%"), cname))
		{
			string1024			tmp;
		...
		HRESULT	const _hr		= ::Render->shader_compile(name,(DWORD const*)data, static_cast<UINT>(size), c_entry, c_target, D3D10_SHADER_PACK_MATRIX_ROW_MAJOR, (void*&)_ps );
...
SGS*	CResourceManager::_CreateGS			(LPCSTR name)
{
	...
	else
	{
		...
		HRESULT	const _hr		= ::Render->shader_compile(name,(DWORD const*)file->pointer(),static_cast<UINT>(file->length()), c_entry, c_target, D3D10_SHADER_PACK_MATRIX_ROW_MAJOR, (void*&)_gs );
...
//--------------------------------------------------------------------------------------------------------------
#ifdef USE_DX11
CRT*	CResourceManager::_CreateRT		(LPCSTR Name, bsize w, bsize h,	D3DFORMAT f, bsize SampleCount, bool useUAV )
#else
CRT*	CResourceManager::_CreateRT		(LPCSTR Name, bsize w, bsize h,	D3DFORMAT f, bsize SampleCount )
#endif
{
