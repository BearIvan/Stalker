...
namespace luabind {
	namespace detail {
		...
		static map_type* g_registered = 0;
		LUABIND_API class_id allocate_class_id(type_id const& cls)
		{
			...
			if (g_registered == 0)g_registered = &registered;
			std::pair<map_type::iterator, bool> inserted = registered.insert(std::make_pair(cls, id));
			if(inserted.second) ++id;

			return inserted.first->second;
		}
		LUABIND_API void free_class_id(type_id const& cls)
		{
			if (g_registered==0)return;
			auto item = g_registered->find(cls);
			if (item != g_registered->end())
			{
				g_registered->erase(item);
			}
		}
	} // namespace detail
} // namespace luabind
