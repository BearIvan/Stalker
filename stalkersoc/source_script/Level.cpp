		...
		if (!psNET_direct_connect) 
		{
			...
			if (IsClient())
			{
				...
				F->OutNext("P(%d), BPS(%2.1fK), MRR(%2d), MSR(%2d), Retried(%2d), Blocked(%2d), Sended(%2d), SPS(%2d)",
					...
					net_Statistic.dwBytesSendedPerSec
					);
			}
		}
	}
...
//	g_pGamePersistent->Environment().SetGameTime	(GetGameDayTimeSec(),GetGameTimeFactor());
	ENV_SOC.SetGameTime	(GetEnvironmentGameDayTimeSec(),GetGameTimeFactor());
...
void CLevel::SetEnvironmentGameTimeFactor(u64 const &GameTime, const float &fTimeFactor)
{
