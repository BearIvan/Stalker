...
void CAI_Space::load				(LPCSTR level_name)
{
	...
	m_graph_engine			= xr_new<CGraphEngine>(
		XrMath::max(
			static_cast<u32>(game_graph().header().vertex_count()),
...
void CAI_Space::validate(const u32 level_id) const
{
	...
	{
		bool b1 = level_id == game_graph().vertex(i)->level_id();
		if (b1 == false)continue;
		bool b2 = level_graph().valid_vertex_id(game_graph().vertex(i)->level_vertex_id());
		bool b3 = cross_table().vertex(game_graph().vertex(i)->level_vertex_id()).game_vertex_id() != i;
		bool b4 = level_graph().inside(game_graph().vertex(i)->level_vertex_id(), game_graph().vertex(i)->level_point());
		if ( !b2 || b3 || !b4)
		{
