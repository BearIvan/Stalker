...
void CGamePersistent::WeathersUpdate()
{
	if (g_pGameLevel && !g_dedicated_server)
	{
		...
		int data_set = (Random.randF()<(1.f - (*dynamic_cast<CEnvironmentSOC*>(Environment())).CurrentEnv.weight)) ? 0 : 1;
		CEnvDescriptorSOC* _env =  (*dynamic_cast<CEnvironmentSOC*>( Environment())).Current[data_set]; VERIFY(_env);
		CEnvAmbientSOC* env_amb = _env->env_ambient;
		if (env_amb) {
			// start sound
			...
			if ((FALSE == bIndoor) && (0 == ambient_particles) && Device.dwTimeGlobal>ambient_effect_next_time) {
				CEnvAmbientSOC::SEffect* eff = env_amb->get_rnd_effect();
				if (eff) {
					 (*dynamic_cast<CEnvironmentSOC*>( Environment())).wind_gust_factor = eff->wind_gust_factor;
		...
		if (bIndoor || Device.dwTimeGlobal >= ambient_effect_stop_time) {
			if (ambient_particles)					ambient_particles->Stop();
			 (*dynamic_cast<CEnvironmentSOC*>( Environment())).wind_gust_factor = 0.f;
		}
