...
void		xrServer::client_Destroy(IClient* C)
{
	...
	if (!alife_client)return;
	//has spectator ?
	CSE_Abstract* pOwner = ((xrClientData*)alife_client)->owner;
	...
	else
	{
		...
		net_Players_disconnected.push_back(alife_client);
		((xrClientData*)alife_client)->Clear();
	};
...
void xrServer::SendUpdatesToAll(IClient*client)
{
	...
	else
	{
		...
		{//all entities
			...
			if (0 == Test.owner)								return;
			if (!Test.net_Ready)							return;
			if (Test.s_flags.is(M_SPAWN_OBJECT_PHANTOM))	return;	// Surely: phantom
			if (!Test.Net_Relevant())						return;
...
{
	m_iCurUpdatePacket = 0;
	pCurUpdatePacket = &(m_aUpdatePackets[0]);
	pCurUpdatePacket->B.count = 0;
	
	net_players.ForEachClientDo(this, &xrServer::SendUpdatesToAll);
...
bool xrServer::CheckAdminRights(const shared_str& user, const shared_str& pass, string512 reason)
{
	...
	if(FS.ExistFile("%user%", "radmins.ltx"))
	{
		...
		if(ini.line_exist("radmins",user.c_str()))
		{
			...
			{
				BearString::Copy			(reason,512,"Access permitted.");
				res				= true;
			}else
			{
				BearString::Copy			(reason, 512, "Access denied. Wrong password.");
			}
		}else
			BearString::Copy			(reason, 512, "Access denied. No such user.");
	}else
		BearString::Copy				(reason, 512, "Access denied.");

	return				res;
}

void xrServer::SendTo_LL			(ClientID ID, void* data, bsize size, u32 dwFlags, u32 dwTimeout)
{
...
{
	struct OnChatMessageS
	{
		s16 team;
		game_PlayerState* Cps;
		xrServer*			m_server;
		NET_Packet* P;
		OnChatMessageS(xrServer*ser) :m_server(ser) {}
		void operator()(IClient*client)
		{
			...
			if (Cps->testFlag(GAME_PLAYER_FLAG_VERY_VERY_DEAD) && !ps->testFlag(GAME_PLAYER_FLAG_VERY_VERY_DEAD))
				return;
			m_server->SendTo(Client->ID, *P);
		}
	};
	OnChatMessageS temp(this);
	temp.team = P->r_s16();
	temp.P = P;
	if (!CL->net_Ready) return;
	temp.Cps = CL->ps;
	net_players.ForEachClientDo(temp);
};
...
void xrServer::PerformCheckClientsForMaxPing(IClient*client)
{
	xrClientData*	Client = (xrClientData*)client;
	game_PlayerState* ps = Client->ps;
...
{
	net_players.ForEachClientDo(this, &xrServer::PerformCheckClientsForMaxPing);
}
