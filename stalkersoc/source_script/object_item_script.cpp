...
ObjectFactory::CLIENT_BASE_CLASS *CObjectItemScript::client_object	() const
{
	...
	try {
		object	=luabind::call_function<ObjectFactory::CLIENT_SCRIPT_BASE_CLASS*, luabind::policy::adopt<0>>( m_client_creator);
	}
	catch(...) {
		return	(0);
	}
...
ObjectFactory::SERVER_BASE_CLASS *CObjectItemScript::server_object	(LPCSTR section) const
{
	...
	try {
		...
		try {
			instance	= xr_new<luabind::object>(luabind::call_function < luabind::object>(m_server_creator, section));
		}
