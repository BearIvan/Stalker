...
void DEBUGFATALERROR1_if_thread_is_running()
{
...
IC	profile_timer_script	operator+	(const profile_timer_script &portion0, const profile_timer_script &portion1)
{
 ...
 IC	luabind::stringstream& operator<<(luabind::stringstream &stream, const profile_timer_script &timer)
 {
 	stream					<< timer.time();
	return					(stream);
}
...
{
	module(L)[
		def( "is_debug", is_debug),
		def( "log", LuaLog),
		def( "error_log", ErrorLog),
		def( "flush", FlushLogs),
		def( "prefetch", prefetch_module),
		def( "verify_if_thread_is_running", DEBUGFATALERROR1_if_thread_is_running),
