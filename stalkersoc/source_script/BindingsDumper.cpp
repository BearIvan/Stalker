...
void BindingsDumper::Print(const char* s) { string_out.append(s); }
void BindingsDumper::Print(const char* s, int len) { string_out.append(s, len); }
void BindingsDumper::Printf(const char* format, ...)
{
    va_list args;
    va_start(args, format);
    string_out.append_printf_va(format, args);
    va_end(args);
}

void BindingsDumper::PrintIndented(const char* s)
{
    string_out.append_printf(
        "%*s"
        "%s",
        GetIdentSize(), "", s);
...
{
    string_out.append_printf("%*s", GetIdentSize(), "");
    va_list args;
    va_start(args, format);
    string_out.append_printf_va(format, args);
    va_end(args);
...
void BindingsDumper::Dump(lua_State* luaState, BearString& out, const Options& opt)
{
    ls = luaState;
    options = opt;
    shiftLevel = 0;
