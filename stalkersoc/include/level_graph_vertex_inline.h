		...
		) const
{
	...
	*/
	...
	if ((r3*r4 > XrMath::EPS) && !XrMath::fis_zero(r3,XrMath::EPS_L) && !XrMath::fis_zero(r4,XrMath::EPS_L))     return (eLineIntersectionNone);
	...
	*/
	...
	if ((r1*r2 > XrMath::EPS) && !XrMath::fis_zero(r1,XrMath::EPS_L) && !XrMath::fis_zero(r2,XrMath::EPS_L))     return (eLineIntersectionNone);

	// Check for equality
	if (XrMath::fis_zero(r1*r2) && XrMath::fis_zero(r3*r4)) return eLineIntersectionEqual;

	/* Line segments intersect: compute intersection point. 
	*/

	denom = a1 * b2 - a2 * b1;
	if (XrMath::fis_zero(denom)) return (eLineIntersectionCollinear);
...
{
	fAngle				= XrMath::XrMath::angle_normalize(fAngle - 0*XrMath::PI_DIV_2);
...
float CLevelGraph::vertex_cover_angle(u32 vertex_id, float inc_angle, _predicate compare_predicate) const
{
	...
	float best_value	= compute_square(best_angle, XrMath::PI_DIV_2, vertex_id);

	for (float angle = inc_angle; angle <= XrMath::PI_MUL_2; angle += inc_angle) {
		float cover = compute_square(angle, XrMath::PI_DIV_2, vertex_id);
