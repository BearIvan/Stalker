...
TEMPLATE_SPECIALIZATION
IC	void CGameLocationSelector::select_location	(const _vertex_id_type start_vertex_id, _vertex_id_type &dest_vertex_id1)
{
	switch (m_selection_type) {
		...
		case eSelectionTypeRandomBranching : {
			if (this->m_graph)
				select_random_location(start_vertex_id,dest_vertex_id1);
			this->m_failed			= this->m_failed && (start_vertex_id == dest_vertex_id1);
			break;
		}
		default :				NODEFAULT;
	}
...
TEMPLATE_SPECIALIZATION
IC	void CGameLocationSelector::select_random_location(const _vertex_id_type start_vertex_id, _vertex_id_type &dest_vertex_id1)
{
	VERIFY						(inherited::m_graph);
	VERIFY						(inherited::m_graph->valid_vertex_id(start_vertex_id));

	if (!inherited::m_graph->valid_vertex_id(m_previous_vertex_id))
	...
	typename GameGraph::TERRAIN_VECTOR::const_iterator	B = vertex_types.begin(), I;
	typename GameGraph::TERRAIN_VECTOR::const_iterator	E = vertex_types.end();

	_Graph::const_iterator		i,e;
	inherited::m_graph->begin				(start_vertex_id,i,e);
	for ( ; i != e; ++i) {
		...
		if ((inherited::m_graph->vertex((*i).vertex_id())->level_id() != ai().level_graph().level_id()))
			continue;
		...
		const u8				*curr_types = inherited::m_graph->vertex((*i).vertex_id())->vertex_type();

		// * подходит по маске
		for (I = B; I != E; ++I)
			if (inherited::m_graph->mask((*I).tMask,curr_types))
				++branch_factor;
	}

	if (!branch_factor) {
		if ((start_vertex_id != m_previous_vertex_id) && accessible(m_previous_vertex_id))
			dest_vertex_id1		= m_previous_vertex_id;
		else
			dest_vertex_id1		= start_vertex_id;
	}
	else {
...
{
	return					(inherited::m_restricted_object ? inherited::m_restricted_object->accessible(inherited::m_graph->vertex(vertex_id)->level_vertex_id()) : true);
}
