...
struct WaveForm
{
	...
	IC float Func		(float t)
	{
		switch (F)
		{
			...
			case fTRIANGLE:		return asinf(XrMath::sin((t-0.25f)*XrMath::XrMath::PI_MUL_2))/XrMath::XrMath::PI_DIV_2;
			case fSQUARE:		return signf(XrMath::cos(t*XrMath::M_PI));
			case fSAWTOOTH:		return atanf(tanf((t+0.5f)*XrMath::M_PI))/XrMath::XrMath::PI_DIV_2;
			case fINVSAWTOOTH:	return -(atanf(tanf((t+0.5f)*XrMath::M_PI))/XrMath::XrMath::PI_DIV_2);
		}
		return 0.f;
	}
public:
