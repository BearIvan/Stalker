	...
	)
{
	...
	goal_vertex				= inherited::graph->vertex(inherited::goal_node_index);
}
...
{
	VERIFY					(inherited::graph);
	return					(goal_vertex->game_point().distance_to(inherited::graph->vertex(node_index)->game_point()));
}
...
{
	VERIFY					(inherited::graph);
	return					(inherited::graph->accessible(vertex_id));
}
