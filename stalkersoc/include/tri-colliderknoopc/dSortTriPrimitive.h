...
///////////////////////////////////////////////////////////////////////////////////////////////	
	...
	for (I=B; I!=E; ++I)
	{
		...
		auto T1 = T_array + *I;
		const Point vertices[3]={Point((dReal*)&V_array[T1->verts[0]]),Point((dReal*)&V_array[T1->verts[1]]),Point((dReal*)&V_array[T1->verts[2]])};
		if(!aabb_tri_aabb(Point(p),Point((float*)&AABB),vertices))
																continue;
#ifdef DEBUG
		if(ph_dbg_draw_mask.test(phDBgDrawIntersectedTries))
										DBG_DrawTri(T1,V_array,D3DCOLOR_XRGB(0,255,0));
		dbg_tries_num++;
#endif
		Triangle	tri;	
		...
		if(tri.dist<0.f){
			...
			if(ph_dbg_draw_mask.test(phDBgDrawNegativeTries))
				DBG_DrawTri(T1,V_array,D3DCOLOR_XRGB(0,0,255));
#endif
			...
			if(ph_dbg_draw_mask.test(phDBgDrawPositiveTries))
				DBG_DrawTri(T1,V_array,D3DCOLOR_XRGB(255,0,0));
#endif	
				if(ret>flags-10) 
							continue;
