...
{
	inherited::add_state(eStatePanic_Run,					xr_new<CStateMonsterPanicRun<_Object> >(obj));
	inherited::add_state(eStatePanic_FaceUnprotectedArea,	xr_new<CStateMonsterLookToUnprotectedArea<_Object> >(obj));
	inherited::add_state(eStatePanic_MoveToHomePoint,		xr_new<CStateMonsterAttackMoveToHomePoint<_Object> >(obj));	
}
...
{
	if (inherited::get_state(eStatePanic_MoveToHomePoint)->check_start_conditions()) {
		this->select_state(eStatePanic_MoveToHomePoint);
		return;
	}

	if (inherited::prev_substate == eStatePanic_Run) this->select_state(eStatePanic_FaceUnprotectedArea);
...
void CStateMonsterPanicAbstract::setup_substates()
{
	...
	if (inherited::current_substate == eStatePanic_FaceUnprotectedArea) {
...
void CStateMonsterPanicAbstract::check_force_state()
{
	if ((inherited::current_substate == eStatePanic_FaceUnprotectedArea)){
		// если видит врага
		if (inherited::object->EnemyMan.get_enemy_time_last_seen() == Device.dwTimeGlobal) {
			this->select_state(eStatePanic_Run);
			return;
		}
		// если получил hit
		if (inherited::object->HitMemory.get_last_hit_time() + 5000 > Device.dwTimeGlobal) {
			this->select_state(eStatePanic_Run);
			return;
		}
	}
