...
{
	inherited::add_state	(eStateSquad_Rest_Idle,				xr_new<CStateMonsterCustomAction<_Object> >	(obj));
	inherited::add_state	(eStateSquad_Rest_WalkAroundLeader,	xr_new<CStateMonsterMoveToPoint<_Object> >	(obj));
}
...
{
	inherited::select_state(Random.randI(2) ? eStateSquad_Rest_Idle : eStateSquad_Rest_WalkAroundLeader);
}
...
void CStateMonsterSquadRestAbstract::setup_substates()
{
	...
	if (inherited::current_substate == eStateSquad_Rest_Idle) {
	...
	if (inherited::current_substate == eStateSquad_Rest_WalkAroundLeader) {
		SStateDataMoveToPoint data;
		CMonsterSquad	*squad =monster_squad().get_squad(inherited::object);
		
		if (inherited::object->control().path_builder().get_node_in_radius(squad->GetLeader()->ai_location().level_vertex_id(), 8.f, LEADER_RADIUS, FIND_POINT_ATTEMPTS, data.vertex)) {
