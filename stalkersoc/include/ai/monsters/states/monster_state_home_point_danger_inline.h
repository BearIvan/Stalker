...
{
	inherited::add_state	(eStatePanic_HomePoint_Hide,			xr_new<CStateMonsterMoveToPointEx<_Object> >	(obj));
	inherited::add_state	(eStatePanic_HomePoint_LookOpenPlace,	xr_new<CStateMonsterLookToPoint<_Object> >		(obj));
	inherited::add_state	(eStatePanic_HomePoint_Camp,			xr_new<CStateMonsterCustomAction<_Object> >		(obj));
}
...
{
	return (!inherited::object->Home->at_home() && !inherited::object->Home->at_home(get_most_danger_pos()));
}
...
bool CStateMonsterDangerMoveToHomePointAbstract::check_completion()
{
	...
	if (m_skip_camp && (inherited::prev_substate != u32(-1)) && (inherited::prev_substate != eStatePanic_HomePoint_Hide) ) return true;
...
void CStateMonsterDangerMoveToHomePointAbstract::reselect_state()
{
	if (inherited::prev_substate == u32(-1)) {
		inherited::select_state(eStatePanic_HomePoint_Hide);
		return;
	} 

	if (inherited::prev_substate == eStatePanic_HomePoint_Hide) {
		inherited::select_state(eStatePanic_HomePoint_LookOpenPlace);
		return;
	}

	inherited::select_state(eStatePanic_HomePoint_Camp);
}
...
void CStateMonsterDangerMoveToHomePointAbstract::setup_substates()
{
	...
	if (inherited::current_substate == eStatePanic_HomePoint_Hide) {
	...
	if (inherited::current_substate == eStatePanic_HomePoint_LookOpenPlace) {
	...
	if (inherited::current_substate == eStatePanic_HomePoint_Camp) {
...
{	
	...
	if (inherited::object->HitMemory.is_hit()) {
		m_danger_pos = inherited::object->HitMemory.get_last_hit_position();
	} else if (inherited::object->hear_dangerous_sound) {
		m_danger_pos = inherited::object->SoundMemory.GetSound().position;
	} 
