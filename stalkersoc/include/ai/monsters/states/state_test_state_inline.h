...
{
	inherited::add_state(eStateCustom,xr_new<CStateMonsterMoveToPointEx<_Object> >(obj));
}

TEMPLATE_SPECIALIZATION
void CStateMonsterTestStateAbstract::reselect_state()
{
	inherited::select_state(eStateCustom);
}
...
void CStateMonsterTestStateAbstract::setup_substates()
{
	...
	if (inherited::current_substate == eStateCustom) {
...
{
	inherited::add_state(eStateAttack_HideInCover,xr_new<CStateMonsterMoveToPointEx<_Object> >(obj));
	inherited::add_state(eStateAttack_CampInCover,xr_new<CStateMonsterCustomAction<_Object> >(obj));
}
TEMPLATE_SPECIALIZATION
void CStateMonsterTestCoverAbstract::initialize()
{
	...
	m_last_node	= inherited::object->m_target_node;
}
...
{
	if (m_last_node	!= inherited::object->m_target_node) {
		m_last_node			= inherited::object->m_target_node;
		inherited::current_substate	= u32(-1);
		return;
	}
	
	if (inherited::current_substate == eStateAttack_CampInCover)
		if (inherited::object->ai_location().level_vertex_id() != m_last_node)
			inherited::current_substate = u32(-1);
}
TEMPLATE_SPECIALIZATION
...
{
	if (inherited::object->ai_location().level_vertex_id() != m_last_node)
		inherited::select_state(eStateAttack_HideInCover);
	else 
		inherited::select_state(eStateAttack_CampInCover);
}
...
void CStateMonsterTestCoverAbstract::setup_substates()
{
	...
	if (inherited::current_substate == eStateAttack_HideInCover) {
		SStateDataMoveToPointEx data;
