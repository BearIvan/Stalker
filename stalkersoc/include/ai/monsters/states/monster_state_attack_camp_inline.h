...
{
	inherited::add_state	(eStateAttackCamp_Hide,		xr_new<CStateMonsterMoveToPointEx<_Object> >	(obj));
	inherited::add_state	(eStateAttackCamp_Camp,		xr_new<CStateMonsterLookToPoint<_Object> >		(obj));
	inherited::add_state	(eStateAttackCamp_StealOut,	xr_new<CStateMonsterAttackCampStealOut<_Object> >(obj));
}
...
{
	if (inherited::current_substate == eStateAttackCamp_StealOut) {
		return inherited::get_state_current()->check_completion();
	}

	if (inherited::current_substate == eStateAttackCamp_Camp) {
		if (inherited::object->EnemyMan.see_enemy_now()) return true;
		if (inherited::object->HitMemory.get_last_hit_time() > inherited::get_state_current()->time_started()) return true;
	}

	if (inherited::object->EnemyMan.get_enemy()->Position().distance_to(inherited::object->Position()) < 5.f) return true;
...
{
	if (!inherited::object->ability_distant_feel()) return false;

	// check enemy
	if (!inherited::object->EnemyMan.get_enemy()) return false;
	
	// check distance to enemy
	if (inherited::object->Position().distance_to(inherited::object->EnemyMan.get_enemy_position()) < MIN_DISTANCE_TO_ENEMY) return false;
...
void CStateMonsterAttackCampAbstract::reselect_state()
{
	if (inherited::prev_substate == u32(-1)) {
		inherited::select_state(eStateAttackCamp_Hide);
		return;
	}

	if (inherited::prev_substate == eStateAttackCamp_Hide) {
		inherited::select_state(eStateAttackCamp_Camp);
		return;
	}

	if (inherited::prev_substate == eStateAttackCamp_Camp) {
		if (inherited::get_state(eStateAttackCamp_StealOut)->check_start_conditions())
			inherited::select_state(eStateAttackCamp_StealOut);
		else 
			inherited::select_state(eStateAttackCamp_Hide);
		return;
	}

	if (inherited::prev_substate == eStateAttackCamp_StealOut) {
		inherited::select_state(eStateAttackCamp_Camp);
		return;
	}
}
...
void CStateMonsterAttackCampAbstract::setup_substates()
{
	...
	if (inherited::current_substate == eStateAttackCamp_Hide) {
