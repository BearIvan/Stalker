...
bool CStateMonsterSmartTerrainTaskGraphWalkAbstract::check_start_conditions()
{
	...
	m_task							= monster->brain().smart_terrain().task(monster);	
	VERIFY3							(m_task, "Smart terrain selected, but task was not set for monster ", *inherited::object->cName());
	if (inherited::object->ai_location().game_vertex_id() == m_task->game_vertex_id()) return false;
...
bool CStateMonsterSmartTerrainTaskGraphWalkAbstract::check_completion()
{
	...
	if (inherited::object->ai_location().game_vertex_id() == m_task->game_vertex_id()) return true;
	return false;
}
...
{
	inherited::object->set_action					(ACT_WALK_FWD);
	inherited::object->set_state_sound				(MonsterSound::eMonsterSoundIdle);

	inherited::object->path().detour_graph_points	(m_task->game_vertex_id());
}
