...
{
	inherited::add_state	(eStateAttack_HomePoint_Hide,			xr_new<CStateMonsterMoveToPointEx<_Object> >	(obj));
	inherited::add_state	(eStateAttack_HomePoint_LookOpenPlace,	xr_new<CStateMonsterLookToPoint<_Object> >		(obj));
	inherited::add_state	(eStateAttack_HomePoint_Camp,			xr_new<CStateMonsterCustomAction<_Object> >		(obj));
}
...
{
	return (!inherited::object->Home->at_home() && !inherited::object->Home->at_home(inherited::object->EnemyMan.get_enemy_position()));
}
...
bool CStateMonsterAttackMoveToHomePointAbstract::check_completion()
{
	...
	if (inherited::object->EnemyMan.see_enemy_now() && inherited::object->Home->at_home(inherited::object->EnemyMan.get_enemy()->Position())) return true;
	if (m_skip_camp && (inherited::prev_substate != u32(-1)) && (inherited::prev_substate != eStateAttack_HomePoint_Hide) ) return true;
...
void CStateMonsterAttackMoveToHomePointAbstract::reselect_state()
{
	if (inherited::prev_substate == u32(-1)) {
		inherited::select_state(eStateAttack_HomePoint_Hide);
		return;
	} 
	
	if (inherited::prev_substate == eStateAttack_HomePoint_Hide) {
		inherited::select_state(eStateAttack_HomePoint_LookOpenPlace);
		return;
	}

	inherited::select_state(eStateAttack_HomePoint_Camp);
}
...
void CStateMonsterAttackMoveToHomePointAbstract::setup_substates()
{
	...
	if (inherited::current_substate == eStateAttack_HomePoint_Hide) {
	...
	if (inherited::current_substate == eStateAttack_HomePoint_LookOpenPlace) {
	...
	if (inherited::current_substate == eStateAttack_HomePoint_Camp) {
