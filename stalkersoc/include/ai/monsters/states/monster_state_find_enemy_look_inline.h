...
{
	inherited::add_state	(eStateFindEnemy_LookAround_MoveToPoint,	xr_new<CStateMonsterMoveToPoint<_Object> >	(obj));
	inherited::add_state	(eStateFindEnemy_LookAround_LookAround,		xr_new<CStateMonsterCustomAction<_Object> > (obj));
	inherited::add_state	(eStateFindEnemy_LookAround_TurnToPoint,	xr_new<CStateMonsterLookToPoint<_Object> >	(obj));
}
...
void CStateMonsterFindEnemyLookAbstract::initialize()
{
	...
	current_dir		= inherited::object->Direction();
	start_position	= inherited::object->Position();
}
...
void CStateMonsterFindEnemyLookAbstract::reselect_state()
{
	if ((current_stage == 1) || (current_stage == 3)) {
		...
		inherited::select_state((Random.randI(2)) ? eStateFindEnemy_LookAround_MoveToPoint : eStateFindEnemy_LookAround_TurnToPoint);

	} else inherited::select_state(eStateFindEnemy_LookAround_LookAround); 
...
void CStateMonsterFindEnemyLookAbstract::setup_substates()
{
	...
	if (inherited::current_substate == eStateFindEnemy_LookAround_MoveToPoint) {
	...
	if (inherited::current_substate == eStateFindEnemy_LookAround_LookAround) {
	...
	if (inherited::current_substate == eStateFindEnemy_LookAround_TurnToPoint) {
