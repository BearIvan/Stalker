...
{
	inherited::add_state	(eStateRest_WalkToCover,	xr_new<CStateMonsterMoveToPointEx<_Object> >	(obj));
	inherited::add_state	(eStateRest_LookOpenPlace,	xr_new<CStateMonsterLookToPoint<_Object> >		(obj));
	inherited::add_state	(eStateRest_Idle,			xr_new<CStateMonsterCustomAction<_Object> >		(obj));
}
...
void CStateMonsterRestIdleAbstract::initialize()
{
	...
	// try to get cover
	const CCoverPoint *point = inherited::object->CoverMan->find_cover(inherited::object->Position(), 5.f, 10.f);
	if (!point) {
		point = inherited::object->CoverMan->find_cover(inherited::object->Position(), 10.f, 30.f);
		if (!point) return;
	}
...
{
	if ((inherited::prev_substate == u32(-1)) && (m_target_node != u32(-1))) {
		inherited::select_state(eStateRest_WalkToCover);
		return;
	} 

	if ((inherited::prev_substate == eStateRest_WalkToCover) || (inherited::prev_substate == u32(-1))) {
		inherited::select_state(eStateRest_LookOpenPlace);
		return;
	}
...
void CStateMonsterRestIdleAbstract::setup_substates()
{
	...
	if (inherited::current_substate == eStateRest_WalkToCover) {
	...
	if (inherited::current_substate == eStateRest_LookOpenPlace) {
	...
	if (inherited::current_substate == eStateRest_Idle) {
