...
{
	inherited::add_state	(eStateControlled_Follow_Wait,			xr_new<CStateMonsterCustomAction<_Object> >	(obj));
	inherited::add_state	(eStateControlled_Follow_WalkToObject,	xr_new<CStateMonsterMoveToPointEx<_Object> >(obj));
}
...
void CStateMonsterControlledFollowAbstract::reselect_state()
{
	...
	float dist = inherited::object->Position().distance_to(target_object->Position());	
	inherited::select_state(dist < Random.randF(STOP_DISTANCE, STAY_DISTANCE) ? eStateControlled_Follow_Wait : eStateControlled_Follow_WalkToObject);
}
...
void CStateMonsterControlledFollowAbstract::setup_substates()
{
	...
	if (inherited::current_substate == eStateControlled_Follow_Wait) {
	...
	if (inherited::current_substate == eStateControlled_Follow_WalkToObject) {
