...
{
	VERIFY					(m_evaluator);
	if (inherited::graph->vertex(inherited::data_storage->get_best().index())->level_id() == m_evaluator->m_level_id) {
		m_evaluator->m_vertex_id	= inherited::data_storage->get_best().index();
		return				(true);
	}
	return					(false);
}
