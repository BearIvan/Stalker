...
{
#ifdef LOG_ACTION
	inherited::m_evaluator_name	= evaluator_name;
#endif
	inherited::m_storage			= storage;
}

TEMPLATE_SPECIALIZATION
void CEvaluator::setup					(_object_type *object, CPropertyStorage *storage)
{
	inherited::setup	(object, inherited::m_storage ? inherited::m_storage : storage);
}

TEMPLATE_SPECIALIZATION
typename CEvaluator::_value_type CEvaluator::evaluate	()
{
	return				((inherited::m_storage->property(m_condition_id) == m_value) == m_equality);
}
