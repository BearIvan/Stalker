...
void	CIKLimbsController::LimbSetup(  const u16 bones[4] )
{
	...
	Fvector toe;
	get_toe(K, toe, bones );
	_bone_chains.back( ).Create( ( u16 ) _bone_chains.size( )-1,K, bones, toe, true );//Fvector( ).set( 0.13143f, 0, 0.20f )
}
...
void CIKLimbsController::Calculate( )
{
	...
	{
		SCalculateData cd(_bone_chains[left_leg], K,obj);//,m_anim_base,m_uneffected
		LimbCalculate(cd);
	}
	
	{
		SCalculateData cd(_bone_chains[right_leg], K,obj);//,m_anim_base,m_uneffected
		LimbCalculate(cd);
	}
/*
	{
...
void  CIKLimbsController:: IKVisualCallback( IKinematics* K )
{
	...
	CGameObject* O=( ( CGameObject* )(static_cast<IKinematics*>(K))->GetUpdateCallbackParam() );
	CPhysicsShellHolder*	Sh = smart_cast<CPhysicsShellHolder*>( O );
	VERIFY( Sh );
	CIKLimbsController* ik = Sh->character_ik_controller( );
	VERIFY( ik );
