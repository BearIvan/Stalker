...
/*	xrClientData *pStartedPlayer = NULL;
	...
	*/
	...
	P.w_stringZ(tmp_functor.pStartedPlayer ? tmp_functor.pStartedPlayer->ps->getName() : "");
...
void		game_sv_mp::UpdateVote				()
{
	...
	{
		if (vote_stats.NumToCount == vote_stats.NumAgreed) VoteSucceed = true;
		else
	...
	else
	{
		if (g_sv_mp_bCountParticipants) 
			VoteSucceed = (float(vote_stats.NumAgreed)/float(vote_stats.NumParticipated)) >= g_sv_mp_fVoteQuota;
		else
			VoteSucceed = (float(vote_stats.NumAgreed)/float(vote_stats.NumToCount)) >= g_sv_mp_fVoteQuota;
	};
...
void	game_sv_mp::SendPlayerKilledMessage	(u16 KilledID, KILL_TYPE KillType, u16 KillerID, u16 WeaponID, SPECIAL_KILL_TYPE SpecialKill)
{
	...
	struct player_killed_sender
	{
		...
		void operator()(IClient* client)
		{
			xrClientData* tmp_client = static_cast<xrClientData*>(client);
			game_PlayerState* ps = tmp_client->ps;
			if (!tmp_client || !tmp_client->net_Ready || !ps) return;
			server_for_send->SendTo(tmp_client->ID, *P);
		}
	};
...
void	game_sv_mp::OnPlayerChangeName		(NET_Packet& P, ClientID sender)
{
	...
	{
		NewPlayerName_Generate(pClient, NewName,1024);
	};
