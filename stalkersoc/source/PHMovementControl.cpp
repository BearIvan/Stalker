...
#ifdef DEBUG
	if(ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject)&&(!!pObject->cName())&&BearString::CompareWithoutCase(PH_DBG_ObjectTrack(),*pObject->cName())==0)
	{
...
void CPHMovementControl::Load					(LPCSTR section){
	...
	//BearString::Copy(m_capture_bone,pSettings->r_string(section,"capture_bone"));
