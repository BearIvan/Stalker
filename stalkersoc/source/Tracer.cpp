...
CTracer::CTracer()
{
	...
	for (u8 i=0; i<255; i++)
	{
		...
		float r, g, b;
		BearCore::BearString::Scanf(pSettings->r_string(TRACERS_COLOR_TABLE, *LineName),	"%f,%f,%f", &r, &g, &b);		
	...
	bool bActor)
{
	if (::Render->ViewBase.testSphere_dirty((Fvector&)center, length*.5f))
	{
		...
		if (bActor)
		{
			...
			FillSprite_Circle(pos, k_speed*width*0.5f, k_speed*width*0.5f, m_aColors[colorID]);
		}
