...
//}
 for (u32 i2=0; i2<ret; ++i2) {
    CONTACT(contact,i2*skip)->g1 = const_cast<dxGeom*> (o2);
    CONTACT(contact,i2*skip)->g2 = const_cast<dxGeom*> (o1);
	CONTACT(contact,i2*skip)->normal[0] = norm[0];
	CONTACT(contact,i2*skip)->normal[1] = norm[1];
	CONTACT(contact,i2*skip)->normal[2] = norm[2];
	SURFACE(contact,i2*skip)->mode=T->T->material;
 }
