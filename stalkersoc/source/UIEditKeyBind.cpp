...
u32 cut_string_by_length(CGameFont* pFont, LPCSTR src, LPSTR dst, u32 dst_size, float length)
{
	if ( pFont->IsMultibyte() ) {
		...
		BearString::CopyWithSizeLimit( dst, dst_size, src , nPos );
		dst[ nPos ] = '\0';
		return nPos;
	} else {
		...
		BearString::Copy								(dst, dst_size,src);
...
void CUIEditKeyBind::OnFocusLost()
{
	...
	m_bEditMode					= false;
	m_lines.SetTextColor		((XrColor::subst_alpha(m_lines.GetTextColor(), XrColor::color_get_A(0xffffffff))));
}
...
bool CUIEditKeyBind::OnMouseDown(int mouse_btn)
{
	...
	{		
		...
		BearString::Copy				(message, m_action->action_name);
		BearString::Contact				(message, "=");
		BearString::Contact				(message, m_keyboard->key_name);		
...
void CUIEditKeyBind::Update()
{
	...
	{
		m_pAnimation->Update();
		m_lines.SetTextColor((XrColor::subst_alpha(m_lines.GetTextColor(), XrColor::color_get_A(m_pAnimation->GetColor()))));
	}
...
void CUIEditKeyBind::OnMessage(const char* message){
	...
	string64			command;
	BearString::Copy				(command, message);
	command[eq]			= 0;
