	...
	switch (shape.type) {
		...
		case 1 : {
			...
			for (int i=0; i<8; ++i) {
				...
				start.x				= XrMath::min(start.x,temp.x);
				start.y				= XrMath::min(start.y,temp.y);
				start.z				= XrMath::min(start.z,temp.z);
				dest.x				= XrMath::max(dest.x,temp.x);
				dest.y				= XrMath::max(dest.y,temp.y);
				dest.z				= XrMath::max(dest.z,temp.z);
			}
			break;
		}
		default : NODEFAULT;
