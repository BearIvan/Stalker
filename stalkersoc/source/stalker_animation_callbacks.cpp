...
void _detail::callback		(CBoneInstance *B)
{
	...
	float					yaw		= XrMath::angle_normalize_signed(-yaw_factor * XrMath::angle_normalize_signed(A->movement().head_orientation().current.yaw + effector_yaw - (A->movement().body_orientation().current.yaw)));
	float					pitch	= XrMath::angle_normalize_signed(-pitch_factor * XrMath::angle_normalize_signed(A->NET_Last.o_torso.pitch + effector_pitch));
	VERIFY					(_valid(yaw));
	VERIFY					(_valid(pitch));
