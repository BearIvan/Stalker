...
void CEncyclopediaArticle::load_shared	(LPCSTR)
{
	...
	if(0==BearCore::BearString::CompareWithoutCase(atricle_type.c_str(),"encyclopedia")){
		data()->articleType = ARTICLE_DATA::eEncyclopediaArticle;
	}else
	if(0==BearCore::BearString::CompareWithoutCase(atricle_type.c_str(),"journal")){
		data()->articleType = ARTICLE_DATA::eJournalArticle;
	}else
	if(0==BearCore::BearString::CompareWithoutCase(atricle_type.c_str(),"task")){
		data()->articleType = ARTICLE_DATA::eTaskArticle;
	}else
	if(0==BearCore::BearString::CompareWithoutCase(atricle_type.c_str(),"info")){
