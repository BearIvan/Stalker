...
void game_sv_Deathmatch::OnRoundStartFunctor(IClient*client)
...
/*	u32		cnt = get_players_count();
	...
	m_server->ForEachClientDo(this,&game_sv_Deathmatch::OnRoundStartFunctor);
}
void game_sv_Deathmatch::OnRoundEndFunctor(IClient*client)
{
	xrClientData *l_pC = (xrClientData*)client;
	game_PlayerState* ps = l_pC->ps;
	if (!ps)				return;
...
void game_sv_Deathmatch::OnRoundEnd()
{
	...
	case GAME_PHASE_INPROGRESS:
		{
		m_server->ForEachClientDo(this,&game_sv_Deathmatch::OnRoundEndFunctor);
		}break;
	}
...
void	game_sv_Deathmatch::GetWinningPlayerFunctor(IClient*client)
{
	xrClientData *l_pC = (xrClientData*)client;
	game_PlayerState* ps = l_pC->ps;
	if (!ps) return;
