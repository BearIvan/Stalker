...
//.	ArtefactsRPoints_ID.clear();
	...
	ArtefactChooserRandom.seed	( BearCore::BearTimer::GetCurrentTime().asmiliseconds());
}
											...
											bool force_find)
{
	...
	for (u32 p = 0; p<rps.size(); ++p)
	{
		...
		if (tmp_blocker.blocked || rp.Blocked)
		{
			continue;
		};
		dest.push_back(p);
			...
			if (ps_who)
			{
				...
				if (!m_bArtefactWasTaken)
				{
					...
					{		
						struct Functor
						{
							game_sv_ArtefactHunt*m_t;
							game_PlayerState*ps_who;
							Functor(game_sv_ArtefactHunt*t):m_t(t) {}
							void operator()(IClient*client)
							{
								...
								m_t->Player_AddExperience(pstate, READ_IF_EXISTS(pSettings, r_float, "mp_bonus_exp", "af_first_take_all", 0));
							}
						}functor(this);
						functor.ps_who = ps_who;
						m_server->ForEachClientDo(functor);
...
void game_sv_ArtefactHunt::OnArtefactOnBase(ClientID id_who)
{
	...
	struct Functor
	{
		...
		void operator()(IClient*client)
		{
			...
			{
				m_t->Player_AddMoney(pstate, pTeam->m_iM_TargetSucceedAll);
				m_t->Player_AddExperience(pstate, READ_IF_EXISTS(pSettings, r_float, "mp_bonus_exp", "target_succeed_all", 0));
			}
			else
			{
				m_t->Player_AddMoney(pstate, pTeam->m_iM_TargetFailed);
				if (!m_t->m_bArtefactWasDropped)
					pstate->experience_New *= READ_IF_EXISTS(pSettings, r_float, "mp_bonus_exp", "target_failed_all_mul", 1.0f);
			};

			m_t->Player_AddExperience(pstate, 0);
			m_t->Player_ExperienceFin(pstate);
		}
	}functor(this);
	Set_RankUp_Allowed(true);
	functor.pTeam = GetTeamData(u8(ps->team));
	functor.ps = ps;
	if (functor.pTeam)
	{
		Player_AddMoney(ps, functor.pTeam->m_iM_TargetSucceed);
...
///	return true;
	...
	*/
	if (functor.TeamAlived[0] == 0 || functor.TeamAlived[1] == 0) return FALSE;
...
void game_sv_ArtefactHunt::RespawnAllNotAlivePlayers()
{
	struct Functor
	{
		...
		void operator()(IClient*client)
		{
			...
			{
				m_t->RespawnPlayer(l_pC->ID, true);
				m_t->SpawnWeaponsForActor(l_pC->owner, ps);
				m_t->Check_ForClearRun(ps);
			};
...
void game_sv_ArtefactHunt::CheckForAnyAlivePlayer()
{
	struct Functor
	{
		...
		bool operator()(IClient*client)
		{
			...
			if (!l_pC->net_Ready || ps->testFlag(GAME_PLAYER_FLAG_VERY_VERY_DEAD) || ps->IsSkip())	return false;
	...
	*/
	...
	if (m_server->FindClient(functor) == 0)
	RespawnAllNotAlivePlayers();
}
...
bool game_sv_ArtefactHunt::CheckAlivePlayersInTeam(s16 Team)
{
	...
	functor.Team = Team;
	m_server->ForEachClientDo(functor);
	if (functor.cnt_exist == 0) return true;
	return functor.cnt_alive != 0;
	/*for		(u32 it=0; it<cnt; ++it)	
	{
		xrClientData *l_pC = (xrClientData*)	m_server->client_Get	(it);
...
void	game_sv_ArtefactHunt::MoveAllAlivePlayers			()
{
	...
	struct Functor
	{
		...
		u8 AliveCount = 0;
		NET_Packet tmpP; 
		Functor(game_sv_ArtefactHunt*t) :m_t(t) { tmpP.B.count = 0; }
		void operator()(IClient*client)
		{
			xrClientData *l_pC = (xrClientData*)client;
			game_PlayerState* ps = l_pC->ps;
			...
			if (!ps->testFlag(GAME_PLAYER_FLAG_ONBASE)) m_t->assign_RP(l_pC->owner, ps);
			//-----------------------------------------------
			...
			//------------------------------------------------		
			NET_Packet	P;
			m_t->u_EventGen(P, GE_ACTOR_MAX_POWER, ps->GameID);
			m_t->m_server->SendTo(l_pC->ID, P, net_flags(TRUE, TRUE));
			//------------------------------------------------
			P.B.count = 0;
			tmpP.w_u16(pA->ID);
	...
	if (functor.AliveCount == 0) return;

	NET_Packet MovePacket;
	MovePacket.w_begin(M_MOVE_PLAYERS);
	MovePacket.w_u8(functor.AliveCount);
	MovePacket.w(&functor.tmpP.B.data, functor.tmpP.B.count);
...
void	game_sv_ArtefactHunt::UpdatePlayersNotSendedMoveRespond()
{
	struct Functor
	{
		...
		void operator()(IClient*client)
		{
			...
			m_t->ReplicatePlayersStateToPlayer(l_pC->ID);
			l_pC->net_PassUpdates = FALSE;
			l_pC->net_LastMoveUpdateTime = Level().timeServer();
		}
	}functor(this);
	...
	if (pWTeam)
	{
		struct Functor
		{
			...
			void operator()(IClient*client)
			{
				...
				m_t->Player_AddMoney(pstate, pWTeam->m_iM_RivalsWipedOut);
			}
		}functor(this);
		functor.WinTeam = WinTeam;
		functor.pWTeam = pWTeam;
		m_server->ForEachClientDo(functor);
	/*	for		(u32 it=0; it<cnt; ++it)	
		{
			// init
