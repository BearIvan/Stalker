...
void CUIDragDropListEx::ReinitScroll()
{
		...
		m_vScrollBar->SetRange		(0, XrMath::max(0,XrMath::iFloor(h1-h2)) );
...
{
	return Ivector2().set	(0, XrMath::iFloor(m_pParentDragDropList->ScrollPos()/float(CellSize().y)));
}
...
Ivector2 CUICellContainer::PickCell(const Fvector2& abs_pos)
{
	...
	ap.sub									(abs_pos);
	ap.mul									(-1);
	res.x									= XrMath::iFloor(ap.x/m_cellSize.x);
	res.y									= XrMath::iFloor(ap.y/m_cellSize.y);
...
/*	FVF::TL* start_pv = (FVF::TL*)RCache.Vertex.Lock((tgt_cells.width() + 1)*(tgt_cells.height() + 1) * 6, hGeom.stride(), vOffset);
	...
	*/
	UIRender->StartPrimitive((tgt_cells.width() + 1)*(tgt_cells.height() + 1) * 6, IUIRender::ptTriList, IUIRender::pttTL);
	for (int x = 0; x <= tgt_cells.width(); ++x) {
		for (int y = 0; y <= tgt_cells.height(); ++y) {
			...
			for (u32 k = 0; k<6; ++k) {
				...
				UIRender->PushPoint(XrMath::iFloor(drawLT.x + p.x*(f_len.x) + f_len.x*x) - 0.5f,
					XrMath::iFloor(drawLT.y + p.y*(f_len.y) + f_len.y*y) - 0.5f, 0, 0xFFFFFFFF, tp.x + uv.x, tp.y + uv.y);
			}
		}
	}
