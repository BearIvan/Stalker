...
{
	xrClientData*	C	= (xrClientData*)m_server->getClient			(it);
	if (0==C)			return 0;
	else				return C->ps;
}
...
struct player_exporter
{
	u32 count;
	ClientID id_to;
	game_PlayerState*	base;
	NET_Packet*			p_to_send;

	player_exporter(ClientID id_to_, game_PlayerState*	base_, NET_Packet*			p_to_send_):id_to(id_to_),base(base_), p_to_send(p_to_send_)
	{
		count = 0;
	}
	void count_players(IClient*client)
	{
		...
		if (!C->net_Ready || (C->ps->IsSkip() && C->ID != id_to)) return;
		count++;
	}

	void export_player(IClient*client)
	{
		string64	p_name;
		xrClientData*	C = (xrClientData*)client;
		game_PlayerState* A = C->ps;;
		if (!C->net_Ready || (A->IsSkip() && C->ID != id_to)) return;
		if (0 == C)	BearString::Copy(p_name, "Unknown");
		else
		{
			CSE_Abstract* C_e = C->owner;
			if (0 == C_e)		BearString::Copy(p_name, "Unknown");
			else
			{
				BearString::Copy(p_name, C_e->name_replace());
			}
		}
...
/*	u32 p_count = 0;
	...
	player_exporter exporter(to, Base, &P);
	m_server->ForEachClientDo(&exporter,&player_exporter::count_players);
	P.w_u16				(u16(exporter.count));
	m_server->ForEachClientDo(&exporter, &player_exporter::export_player);
...
inline void CalcPing(IClient*clinet)
{
	xrClientData*	C = (xrClientData*)clinet;
...
void game_sv_GameState::Update		()
{
	...
	m_server->ForEachClientDo(CalcPing);
...
bool game_sv_GameState::NewPlayerName_Exists( void* pClient, LPCSTR NewName )
{
	...
	struct Functor
	{
		IClient* CL;
		LPCSTR NewName;
		Functor(IClient* CL_, LPCSTR NewName_) :CL(CL_), NewName(NewName_)
		{

		}
		bool operator()(IClient*pIC)
		{
			if (!pIC || pIC == CL) return false;
			string64 xName;
			BearString::Copy(xName, pIC->name.c_str());
...
void game_sv_GameState::NewPlayerName_Generate( void* pClient, LPSTR NewPlayerName,bsize size )
{
	...
	for ( int i = 1; i < 100; ++i )
	{
		...
		{
			BearString::Copy( NewPlayerName,size, NewXName );
			return;
		}
	}
}
...
void game_sv_GameState::SaveMapList				()
{
	...
	IWriter*		fs	=XRayBearWriter::Create( FS.Write("%user%", MAPROT_LIST_NAME,0));
...
//	u32 TeamColorsDist[TEAM_COUNT] = {XrColor::color_argb(128, 255, 0, 0), XrColor::color_argb(128, 0, 255, 0), XrColor::color_argb(128, 0, 0, 255), XrColor::color_argb(128, 255, 255, 0)};
