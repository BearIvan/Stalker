...
//			font.OutNext		("%s",stalker->name_replace());
			...
			Fvector				position1 = convert_position(graph.vertex(stalker->m_tGraphID)->game_point());
...
//			font.OutNext		("%s",monster->name_replace());
			...
			Fvector				position1 = convert_position(graph.vertex(monster->m_tGraphID)->game_point());
			render.draw_aabb	(position1,radius,radius,radius,color);
			first_time			= false;
			continue;
		}
