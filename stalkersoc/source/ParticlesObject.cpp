...
{
	if((g_dedicated_server||(!penable)))		return;
...
void CParticlesObject::shedule_Update	(u32 _dt)
{
	...
	if (dt)							{
		if (0){//.psDeviceFlags.test(mtParticles))	{    //. AlexMX comment this line// NO UNCOMMENT - DON'T WORK PROPERLY
			mt_dt					= dt;
			XrFastDelegate<void>		delegate	(this,&CParticlesObject::PerformAllTheWork_mt);
...
{ 
	if((g_dedicated_server||(!penable)))		return 5.0f;
...
{
	if((g_dedicated_server||(!penable)))		return false;
