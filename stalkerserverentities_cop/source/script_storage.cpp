...
#include <stdarg.h>
#include "../StalkerTools/doug_lea_allocator.h"
...
#include "../StalkerTools/memory_allocator_options.h"
...
bool CScriptStorage::load_file_into_namespace(LPCSTR caScriptName, LPCSTR caNamespaceName)
{
	...
	DEBUGFATALERROR1			(lua_gettop(lua()) == start);
	return			(true);
}
...
bool CScriptStorage::namespace_loaded(LPCSTR N, bool remove_from_stack)
{
	...
	for (;;) { 
		if (!xr_strlen(S)) {
			DEBUGFATALERROR1			(lua_gettop(lua()) >= 1);
			lua_pop			(lua(), 1); 
			DEBUGFATALERROR1			(start == lua_gettop(lua()));
			return			(false); 
		}
		LPSTR				S1 = strchr(S,'.'); 
		if (S1)
		...
		if (lua_isnil(lua(),-1)) { 
//			lua_settop		(lua(),0);
			DEBUGFATALERROR1			(lua_gettop(lua()) >= 2);
			lua_pop			(lua(),2); 
			DEBUGFATALERROR1			(start == lua_gettop(lua()));
...
//				lua_settop	(lua(),0);
	...
	if (!remove_from_stack) {
		DEBUGFATALERROR1				(lua_gettop(lua()) == start + 1);
	}
	else {
		DEBUGFATALERROR1				(lua_gettop(lua()) >= 1);
		lua_pop				(lua(),1);
...
bool CScriptStorage::object	(LPCSTR identifier, int type)
{
	...
	while (lua_next(lua(), -2)) {
		...
		if ((lua_type(lua(), -1) == type) && !xr_strcmp(identifier,lua_tostring(lua(), -2))) { 
			DEBUGFATALERROR1			(lua_gettop(lua()) >= 3);
			lua_pop			(lua(), 3); 
			DEBUGFATALERROR1			(lua_gettop(lua()) == start - 1);
			return			(true); 
		} 
...
{	
	...
	if (!xr_strcmp(S,"cannot resume dead coroutine")) {
		DEBUGFATALERROR12	("Please do not return any values from main!!!",caScriptFileName);
#ifdef USE_DEBUGGER
#	ifndef USE_LUA_STUDIO
