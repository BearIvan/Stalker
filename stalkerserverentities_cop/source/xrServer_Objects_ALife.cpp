...
#else // XRGAME_EXPORTS
#	include "../StalkerEngine16/bone.h"
#	include "../StalkerEngine16/render.h"
...
struct logical_string_predicate {
	...
	{
		DEBUGFATALERROR1			(pszA);
		DEBUGFATALERROR1			(buffer);
		DEBUGFATALERROR1			(buffer_size);

		u32				cCharacters =  xr_strlen(pszA)+1;
		DEBUGFATALERROR1			(cCharacters*2 <= buffer_size);
...
struct story_name_predicate {
	...
	{
		DEBUGFATALERROR1	(_1.name.size());
		DEBUGFATALERROR1	(_2.name.size());
...
void	SFillPropData::load			()
    ...
    // spawn story names
	{
		DEBUGFATALERROR1					(spawn_story_names.empty());
	...
	DEBUGFATALERROR1					(character_profiles.empty());
...
{
    DEBUGFATALERROR1					(counter < 0xffffffff);
...
{
	DEBUGFATALERROR1						(m_alife_simulator);
...
void CSE_ALifeCar::data_load(NET_Packet	&tNetPacket)
{
	...
	//DEBUGFATALERROR1(door_states.empty());
