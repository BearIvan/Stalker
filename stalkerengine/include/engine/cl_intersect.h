...
IC bool TestRayTri2(const Fvector& C, const Fvector& D, Fvector* p, float& range)
{
    ...
    if ( XrMath::abs(det) < XrMath::EPS_S) { range = -1; return false; }
    inv_det = 1.0f / det;
    tvec.sub(C, p[0]); // calculate distance from vert0 to ray origin
    u = tvec.dotproduct(pvec)*inv_det; // calculate U parameter and test bounds
    qvec.crossproduct(tvec, edge1); // prepare to test V parameter
...
IC bool TestBBoxTri(const Fmatrix33& A, const Fvector& T, const Fvector& extA, Fvector** p, BOOL bCulling)
{
    ...
    float R =  XrMath::abs(extA.x*A0dN) +  XrMath::abs(extA.y*A1dN) +  XrMath::abs(extA.z*A2dN);
    ...
    R =  XrMath::abs(extA.y*A2dE0) +  XrMath::abs(extA.z*A1dE0);
    ...
    R =  XrMath::abs(extA.y*A2dE1) +  XrMath::abs(extA.z*A1dE1);
    ...
    R =  XrMath::abs(extA.y*A2dE2) +  XrMath::abs(extA.z*A1dE2);
    ...
    R =  XrMath::abs(extA.x*A2dE0) +  XrMath::abs(extA.z*A0dE0);
    ...
    R =  XrMath::abs(extA.x*A2dE1) +  XrMath::abs(extA.z*A0dE1);
    TESTV2(A1xE1dD, -A1dN, R); //AXIS_A1xE1

    // axis C+t*A1xE2
    float A0dE2 = A0dE1 - A0dE0;
    float A1xE2dD = A1xE1dD - A1xE0dD;
    R =  XrMath::abs(extA.x*A2dE2) +  XrMath::abs(extA.z*A0dE2);
    ...
    R =  XrMath::abs(extA.x*A1dE0) +  XrMath::abs(extA.y*A0dE0);
    ...
    R =  XrMath::abs(extA.x*A1dE1) +  XrMath::abs(extA.y*A0dE1);
    TESTV2(A2xE1dD, -A2dN, R); //AXIS_A2xE1

    // axis C+t*A2xE2
    float A2xE2dD = A2xE1dD - A2xE0dD;
    R =  XrMath::abs(extA.x*A1dE2) +  XrMath::abs(extA.y*A0dE2);
...
IC bool TestSphereOBB(const Fsphere& rkSphere, const Fobb& rkBox)
{
    ...
    float fAx =  XrMath::abs(kCDiff.dotproduct(rkBox.m_rotate.i));
    float fAy =  XrMath::abs(kCDiff.dotproduct(rkBox.m_rotate.j));
    float fAz =  XrMath::abs(kCDiff.dotproduct(rkBox.m_rotate.k));
...
IC bool TestRayOBB(const Fvector3& origin, const Fvector3& direction, const Fobb& rkBox)
{
    ...
    fAWxDdU[0] =  XrMath::abs(kWxD.dotproduct(rkBox.m_rotate.i));
    fRhs = rkBox.m_halfsize[1] * fAWdU[2] + rkBox.m_halfsize[2] * fAWdU[1];
    if (fAWxDdU[0] > fRhs)
        return false;

    fAWxDdU[1] =  XrMath::abs(kWxD.dotproduct(rkBox.m_rotate.j));
    fRhs = rkBox.m_halfsize[0] * fAWdU[2] + rkBox.m_halfsize[2] * fAWdU[0];
    if (fAWxDdU[1] > fRhs)
        return false;

    fAWxDdU[2] =  XrMath::abs(kWxD.dotproduct(rkBox.m_rotate.k));
