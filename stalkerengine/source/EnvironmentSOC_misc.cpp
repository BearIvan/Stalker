...
//-----------------------------------------------------------------------------
void	CEnvModifierSOC::load	(IReader* fs)
{
//	Fvector			dummy;
...
float	CEnvModifierSOC::sum	(CEnvModifierSOC& M, Fvector3& view)
{
...
// Environment ambient
//-----------------------------------------------------------------------------
void CEnvAmbientSOC::load(const shared_str& sect)
{
	section				= sect;
	string_path			tmp;
	// sounds
...
const char * CEnvDescriptorSOC::getEvnSkyName()
{
	return sky_texture_env_name.c_str();
}

const char * CEnvDescriptorSOC::getSkyName()
{
	return  sky_texture_name.c_str();
}

const char * CEnvDescriptorSOC::getCloudsName()
{
...
//-----------------------------------------------------------------------------
CEnvDescriptorSOC::CEnvDescriptorSOC()
{
	exec_time			= 0.0f;
...
{
	Ivector3 tm				={0,0,0};
	BearString::Scanf					(exec_tm,"%d:%d:%d",&tm.x,&tm.y,&tm.z);
...
void CEnvDescriptorSOC::on_device_create	()
{
...
void CEnvDescriptorSOC::on_device_destroy	()
{
...
// Environment Mixer
//-----------------------------------------------------------------------------
void CEnvDescriptorMixerSOC::destroy()
{
...
void CEnvDescriptorMixerSOC::clear	()
{
...
void CEnvDescriptorMixerSOC::lerp	(CEnvironmentSOC* , CEnvDescriptorSOC& A, CEnvDescriptorSOC& B, float f, CEnvModifierSOC& M, float m_power)
{
...
// Environment IO
//-----------------------------------------------------------------------------
CEnvAmbientSOC* CEnvironmentSOC::AppendEnvAmb		(const shared_str& sect)
{
	for (EnvAmbVecIt it=Ambients.begin(); it!=Ambients.end(); it++)
		if ((*it)->name().equal(sect)) return *it;
	Ambients.push_back		(xr_new<CEnvAmbientSOC>());
	...
	if (WeatherFXs.empty()){
		...
		for (bsize w_idx=0; w_idx<line_count; w_idx++){
			...
			if (pSettings->r_line("weather_effects",w_idx,&weather,&sect_w)){
				...
				env.push_back	(xr_new<CEnvDescriptorSOC>()); env.back()->exec_time_loaded = DAY_LENGTH;
			}
		}
		// sorting weather envs
