...
void CCameraManager::UpdateFromCamera(const CCameraBase* C)
{
	if(gameVersionController->getGame() == gameVersionController->SOC)
		 Update(C->vPosition, C->vDirection, C->vNormal, C->f_fov, C->f_aspect, ENV_SOC.CurrentEnv.far_plane, C->m_Flags.flags);
	else 
		Update(C->vPosition, C->vDirection, C->vNormal, C->f_fov, C->f_aspect,ENV.CurrentEnv->far_plane, C->m_Flags.flags);
}
...
void CCameraManager::UpdatePPEffectors()
{
    ...
    if (m_EffectorsPP.size())
    {
        bool b = false;
        pp_affected = pp_identity;
        for (bsize i = m_EffectorsPP.size() - 1; i >= 0; --i)
        {
