...
#define FAR_DIST ENV.CurrentEnv->far_plane
#define FAR_DIST_SOC ENV_SOC.CurrentEnv->far_plane
...
struct STranspParamSOC {
	...
	float				vis;
	float				vis_threshold;
	STranspParamSOC(CLensFlare* p, const Fvector& _P, const Fvector& _D, float _f, float _vis_threshold) :P(_P), D(_D), f(_f), parent(p), vis(1.f), vis_threshold(_vis_threshold) {}
};
IC BOOL material_callbackSOC(collide::rq_result& result, LPVOID params)
{
	STranspParamSOC* fp = (STranspParamSOC*)params;
	float vis = 1.f;
	if (result.O) {
		vis = 0.f;
		...
		if (fis_zero(vis)) {
			...
			fp->parent->getRayChache().verts[1].set(V[T->verts[1]]);
			fp->parent->getRayChache().verts[2].set(V[T->verts[2]]);
		}
	}
	fp->vis *= vis;
...
void CLensFlare::OnFrame(int id)
{
	...
	dwFrame = Device.dwFrame;
	VERIFY(gameVersionController->getGame() == gameVersionController->SOC);
	vSunDir.mul(ENV_SOC.CurrentEnv.sun_dir, -1);

	// color
	float tf = ENV_SOC.fTimeFactor;
	Fvector& c = ENV_SOC.CurrentEnv.sun_color;
	LightColor.set(c.x, c.y, c.z, 1.f);
	...
	if (m_ray_cache->result&&m_ray_cache->similar(TP.P, TP.D, TP.f)) {
		...
		if (CDB::TestRayTri(TP.P, TP.D, m_ray_cache->verts, _u, _v, _range, false) && (_range>0 && _range<TP.f)) {
			...
			// cache outdated. real query.
			r_dest.r_clear();
			if (g_pGameLevel->ObjectSpace.RayQuery(r_dest, RD, material_callbackSOC, &TP, NULL, o_main))
				m_ray_cache->result = FALSE;
		}
	}
	blend_lerp(fBlend, TP.vis, BLEND_DEC_SPEED, Device.fTimeDelta);
