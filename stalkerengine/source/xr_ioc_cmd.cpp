...
public:
    ...
    virtual void Execute(LPCSTR args) { XrStringContainer::verify(); }
};
...
public:
    ...
    virtual void Execute(LPCSTR args) {XrStringContainer::dump(); }
};
...
public:
    ...
    virtual void Execute(LPCSTR args)
    {
        ...
        // 1..3 - r2
        // 4 - r3
        psDeviceFlags.set(rsR2, ((renderer_value > 1) && renderer_value < 5));
        psDeviceFlags.set(rsR3, (renderer_value == 5));
        psDeviceFlags.set(rsR4, (renderer_value == 6));
		psDeviceFlags.set(rsR5, (renderer_value == 1));
        r2_sun_static = (renderer_value < 3);

        r2_advanced_pp = (renderer_value >= 4);
    }
...
class CCC_EngineVersion : public IConsole_Command
{
public:
...
void CCC_Register()
{
	...
	XrMath::clamp(psSoundOcclusionScale, 0.1f, .5f);
	...
	CMD1(CCC_EngineVersion, "version");
