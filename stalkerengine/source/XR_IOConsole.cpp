...
void CConsole::OutFont(LPCSTR text, float& pos_y)
{
    ...
    if (str_length > scr_width) //1024.0f
    {
        ...
        int sz = 0;
        int ln = 0;
        PSTR one_line = (PSTR)malloc((CONSOLE_BUF_SIZE + 1) * sizeof(char));
...
void CConsole::DrawBackgrounds(bool bGame)
{
    ...
    if (m_select_tip < (int)m_tips.size())
    {
        Frect r1;
        xr_string tmp;
        vecTipsEx::iterator itb1 = m_tips.begin() + m_start_tip;
        vecTipsEx::iterator ite1 = m_tips.end();
        for (u32 i = 0; itb1 != ite1; ++itb1, ++i) // tips
        {
            TipString const& ts = (*itb1);
            ...
            r1.x1 = pr.x1 + w1 + pFont->SizeOf_(tmp.c_str());
            r1.y1 = pr.y1 + i * font_h;

            tmp.assign(ts.text.c_str(), ts.HL_finish);
            r1.x2 = pr.x1 + w1 + pFont->SizeOf_(tmp.c_str());
            r1.y2 = r1.y1 + font_h;

            DrawRect(r1, tips_word_color);
...
void CConsole::update_tips()
{
    ...
    PSTR first = (PSTR)malloc((cur_length + 1) * sizeof(char));
