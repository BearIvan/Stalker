...
void engine_impl::weather(LPCSTR value)
{
	...
	CEnvironment& environment = (*dynamic_cast<CEnvironment*>(g_pGamePersistent->Environment()));
    ...
    (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).SelectEnvs(game_time);
}
...
LPCSTR engine_impl::weather()
{
    ...
    return ((*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).GetWeather().c_str());
}
...
LPCSTR engine_impl::current_weather_frame()
{
    ...
    if (!(*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).Current[0])
        return ("");

    return ((*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).Current[0]->m_identifier.c_str());
}
...
float engine_impl::track_frame()
{
    ...
    float start_time = environment.Current[0]->exec_time;
    float stop_time = environment.Current[1]->exec_time;
    float current_time = (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).GetGameTime();
    if (start_time >= stop_time)
    {
...
void engine_impl::track_weather(float const& time)
{
    ...
    bool paused = (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).m_paused;

    (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).m_paused = false;
    (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).SetGameTime(time * 24 * 60 * 60, (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).fTimeFactor);
    (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).m_paused = true;
    (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).SetGameTime(time * 24 * 60 * 60, (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).fTimeFactor);

    (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).m_paused = paused;

    float weight;
    (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).Invalidate();
    (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).lerp(weight);
}

float engine_impl::track_weather()
{
    return ((*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).GetGameTime() / (24 * 60 * 60));
}
...
{
    (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).m_paused = value;
}

bool engine_impl::weather_paused()
{
    return ((*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).m_paused);
}
...
void engine_impl::weather_time_factor(float const& value_raw)
{
    ...
    if (g_pGameLevel && g_pGamePersistent)
        g_pGameLevel->SetEnvironmentGameTimeFactor(iFloor((*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).GetGameTime()*1000.f), value);

    if (g_pGamePersistent)
        (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).fTimeFactor = value;
}
...
float engine_impl::weather_time_factor()
{
    ...
    return ((*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).fTimeFactor);
}
...
{
    return ((*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).CurrentEnv->m_identifier.c_str());
}
...
void engine_impl::weather_current_time(char const* time)
{
    ...
    (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).m_paused = false;
    (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).SetGameTime(float(hours * 60 * 60 + minutes * 60 + seconds), (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).fTimeFactor);
    (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).m_paused = paused;
...
void engine_impl::reload_current_weather()
{
    ...
    float const game_time = (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).GetGameTime();
    manager.weathers().reload_current_weather();
    g_pGameLevel->SetEnvironmentGameTimeFactor(iFloor(game_time), environment.fTimeFactor);
    (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).Current[0] = 0;
    (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).Current[1] = 0;
    (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).SelectEnvs(game_time);
    VERIFY((*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).Current[1]);
    if ((*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).Current[1]->exec_time == game_time)
        (*dynamic_cast<CEnvironment*>( g_pGamePersistent->Environment())).SelectEnvs(game_time + .1f);
}
