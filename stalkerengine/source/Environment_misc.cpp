...
const char * CEnvDescriptor::getEvnSkyName()
{
 VERIFY(gameVersionController->getGame() != gameVersionController->SOC);
	return sky_texture_env_name.c_str();
}

const char * CEnvDescriptor::getSkyName()
{
 VERIFY(gameVersionController->getGame() != gameVersionController->SOC);
	return  sky_texture_name.c_str();
}

const char * CEnvDescriptor::getCloudsName()
{
 VERIFY(gameVersionController->getGame() != gameVersionController->SOC);
...
void CEnvironment::load_weather_effects()
{
    ...
    for (; i != e; ++i)
    {
        ...
        shared_str n_00("00:00:00");
        env.push_back(create_descriptor(n_00, 0));
        ...
        CInifile::Destroy(config);
        shared_str n_24("24:00:00");
        env.push_back(create_descriptor(n_24, 0));
