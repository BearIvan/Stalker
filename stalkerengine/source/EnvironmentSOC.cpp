...
bool CEnvironmentSOC::SetWeatherFX(shared_str name)
{
	...
	if (name.size()) {
		...
		CEnvDescriptorSOC* C1 = CurrentWeather->at(1);
		CEnvDescriptorSOC* CE = CurrentWeather->at(CurrentWeather->size() - 2);
		CEnvDescriptorSOC* CT = CurrentWeather->at(CurrentWeather->size() - 1);
...
IC bool lb_env_pred(const CEnvDescriptorSOC* x, float val)
{
	return x->exec_time < val;
}

void CEnvironmentSOC::SelectEnv(EnvVec* envs, CEnvDescriptorSOC*& e, float gt)
{
...
void CEnvironmentSOC::SelectEnvs(EnvVec* envs, CEnvDescriptorSOC*& e0, CEnvDescriptorSOC*& e1, float gt)
{
	EnvIt env = std::lower_bound(envs->begin(), envs->end(), gt, lb_env_pred);
	if (env == envs->end()) {
		e0 = *(envs->end() - 1);
...
void CEnvironmentSOC::OnFrame()
{
	...
	float	mpower = 0;
	for (xr_vector<CEnvModifierSOC>::iterator mit = Modifiers.begin(); mit != Modifiers.end(); mit++)
