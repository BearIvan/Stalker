...
void CEnvelope::LoadA(IReader& F)
{
    ...
    if (strstr(buf, "{ Envelope"))
    {
        ...
        for (u32 i = 0; i < keys.size(); i++)
        {
            ...
            st_Key& K = *keys[i];
            F.r_string(buf, sizeof(buf));
            int cnt = BearString::Scanf(buf, "Key %f %f %f %f %f %f %f %f %f", f + 0, f + 1, f + 2, f + 3, f + 4, f + 5, f + 6, f + 7, f + 8);
            R_ASSERT(cnt == 9);
            K.value = f[0];
            K.time = f[1];
            K.shape = (u8)f[2];
        ...
        // behavior <pre> <post>
        F.r_string(buf, sizeof(buf));
        int cnt = BearString::Scanf(buf, "Behaviors %d %d", behavior[0], behavior[1]);
        R_ASSERT(cnt == 2);
    }
}
