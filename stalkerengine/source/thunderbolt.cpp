...
void SThunderboltCollection::load(CInifile* pIni, CInifile* thunderbolts, LPCSTR sect)
{
    ...
    for (bsize tb_idx = 0; tb_idx < tb_count; tb_idx++)
    {
        LPCSTR N, V;
        if (pIni->r_line(sect, tb_idx, &N, &V))
            palette.push_back(ENV.thunderbolt_description(*thunderbolts, N));
    }
}
SThunderboltCollection::~SThunderboltCollection()
{
...
void CEffect_Thunderbolt::OnFrame(bsize id, float period, float duration)
{
	...
	if (state == stWorking) {
		...
		ENV_SOC.CurrentEnv.fog_color.mad(fClr, p_fog_color);

		if (::Render->get_generation() == IRender_interface::GENERATION_R2) {
			ENV_SOC.CurrentEnv.sun_dir = current_direction;
			VERIFY2(ENV_SOC.CurrentEnv.sun_dir.y < 0, "Invalid sun direction settings while CEffect_Thunderbolt");
...
void CEffect_Thunderbolt::Bolt(shared_str id, float period, float lt)
{
    ...
    current = ENV.thunderbolt_collection(collection, id)->GetRandomDesc();
    VERIFY(current);
...
void CEffect_Thunderbolt::OnFrame(shared_str id, float period, float duration)
{
    ...
    if (state == stWorking)
    {
        ...
        if (::Render->get_generation() == IRender_interface::GENERATION_R2)
        {
			...
			ENV.CurrentEnv->sun_dir = current_direction;
            VERIFY2(ENV.CurrentEnv->sun_dir.y < 0, "Invalid sun direction settings while CEffect_Thunderbolt");
