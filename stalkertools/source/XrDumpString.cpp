#include "stdafx.h"
#ifdef DEBUG
std::string XrDumpString::get_string(const Fvector& v)
{
    return make_string("( %f, %f, %f )", v.x, v.y, v.z);
}
std::string XrDumpString::get_string(const Fmatrix& dop)
{
    return make_string("\n%f,%f,%f,%f\n%f,%f,%f,%f\n%f,%f,%f,%f\n%f,%f,%f,%f\n",
...
std::string XrDumpString::get_string(const Fbox& box)
{
    return make_string("[ min: %s - max: %s ]", get_string(box.min).c_str(), get_string(box.max).c_str());
}
std::string XrDumpString::get_string(bool v)
{
...
std::string XrDumpString::dump_string(LPCSTR name, const Fvector& v)
{
    return make_string("%s : (%f,%f,%f) ", name, v.x, v.y, v.z);
}

void XrDumpString::dump(LPCSTR name, const Fvector& v)
{
    Msg("%s", dump_string(name, v).c_str());
}

std::string XrDumpString::dump_string(LPCSTR name, const Fmatrix& form)
{
    return
...
void XrDumpString::dump(LPCSTR name, const Fmatrix& form)
{
