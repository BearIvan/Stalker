...
{
    int i, c, len_c, r, s, last_match_length;
    ...
    for (i = s; i < r; i++)
        text_buf[i] = 0x20;
    for (len_c = 0; len_c < F && (c = fs._getb()) != EOF; len_c++)
        text_buf[r + len_c] = (unsigned char)c;
    textsize = len_c;
    for (i = 1; i <= F; i++)
        InsertNode(r - i);
    InsertNode(r);
...
bsize XrCompressor::LZWrite(int hf, void * d, bsize size)
{
	u8* start = (u8*)d;
...
bsize XrCompressor::LZRead(int hf, void *& d, bsize size)
{

	u8* data = (u8*)xr_malloc(size);
	_read(hf, data, static_cast<uint32>( size));
...
void XrCompressor::LZCompress(u8 ** dest, bsize * dest_sz, void * src, bsize src_sz)
{
	u8* start = (u8*)src;
...
void XrCompressor::LZDecompress(u8 ** dest, bsize * dest_sz, void * src, bsize src_sz)
{
	u8* start = (u8*)src;
	fs.Init_Input(start, start + src_sz);
	Decode();
