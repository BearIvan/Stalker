...
{
	DEBUGFATALERROR12						(table.type() == LUA_TTABLE, "invalid loophole description passed");
	luabind::object				result = table[identifier];
	DEBUGFATALERROR12						(result.type() != LUA_TNIL, make_string("cannot read boolean value %s", identifier));
	DEBUGFATALERROR12						(result.type() == LUA_TBOOLEAN, make_string("cannot read boolean value %s", identifier));
...
BOOL is_combat_cover			(shared_str const &table_id)
{
	...
	DEBUGFATALERROR12						(result, make_string("bad or missing description in smart_cover [%s]", table_id.c_str()));
	if (table.type() != LUA_TTABLE) {
		DEBUGFATALERROR1					(table.type() != LUA_TNIL);
		return					(TRUE);
	}
...
LPCSTR parse_string(luabind::object const &table, LPCSTR identifier)
{
	DEBUGFATALERROR12			(table.type() == LUA_TTABLE, "invalid loophole description passed");
	luabind::object	result = table[identifier];
	DEBUGFATALERROR12			(result.type() != LUA_TNIL, make_string("cannot read string value %s", identifier));
	DEBUGFATALERROR12			(result.type() == LUA_TSTRING, make_string("cannot read string value %s", identifier));
...
Fvector parse_fvector (luabind::object const &table, LPCSTR identifier)
{
	DEBUGFATALERROR12			(table.type() == LUA_TTABLE, "invalid loophole description passed");
	luabind::object	result = table[identifier];
	DEBUGFATALERROR12			(result.type() != LUA_TNIL, make_string("cannot read vector value %s", identifier));
					 ...
					 )
{
	...
	DEBUGFATALERROR12			(lua_result.type() != LUA_TNIL, make_string("cannot read number value %s", identifier));
	DEBUGFATALERROR12			(lua_result.type() == LUA_TNUMBER, make_string("cannot read number value %s", identifier));
	float			result = luabind::object_cast<float>(lua_result);
	DEBUGFATALERROR12			(result >= min_threshold, make_string("invalid read number value %s", identifier));
	DEBUGFATALERROR12			(result <= max_threshold, make_string("invalid number value %s", identifier));
	return			(result);
}
...
void parse_table	(luabind::object const &table, LPCSTR identifier, luabind::object &result)
{
	...
	DEBUGFATALERROR12			(result.type() != LUA_TNIL, make_string("cannot read table value %s", identifier));
	DEBUGFATALERROR12			(result.type() == LUA_TTABLE, make_string("cannot read table value %s", identifier));
}
...
void CSE_SmartCover::set_enterable (shared_str const &id)
{
	...
	//DEBUGFATALERROR12						(found != m_draw_data.end(), id.c_str());
...
void CSE_SmartCover::check_enterable_loopholes(shared_str const &description)
{
	string256					temp;
	xr_strcpy					(temp, "smart_covers.descriptions.");
	BearCore::BearString::Contact					(temp, m_description.c_str());
	...
	DEBUGFATALERROR12						(result, make_string("bad or missing transitions table in smart_cover [%s]", temp));
...
shared_str animation_id(luabind::object table)
{
	...
	for ( ; i != e; ++i ) {
		...
		if (string.type() != LUA_TSTRING) {
			DEBUGFATALERROR1	(string.type() != LUA_TNIL);
			continue;
		}
...
void CSE_SmartCover::load_draw_data () {
	...
	if (!result) {
		...
		return;
		//		DEBUGFATALERROR12					(result, make_string("bad or missing loopholes table in smart_cover [%s]", temp));
	}
...
/*		luabind::object	transitions;
		...
		for ( ; I != E; ++I) {
			...
			DEBUGFATALERROR12				(transition.type() == LUA_TTABLE, "invalid loophole description passed");
