...
//.	const SInventoryOwner	&partner = *_partner;
	...
	result			= XrMath::iFloor(result * func(smart_cast<const CGameObject*>(pThis.inv_owner)->ID()));
	//if(result>500)
	//	result		= XrMath::iFloor(result/10+0.5f)*10;

	XrMath::clamp<u32>					(result, 1, 1000000);
	return					(result);
}
