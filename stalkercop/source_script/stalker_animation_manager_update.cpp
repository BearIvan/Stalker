	...
	bool CStalkerAnimationManager::script_callback			() const
{
	...
	bool CStalkerAnimationManager::need_update				() const
{
	if (script_callback())
		return				(true);
	...
	void CStalkerAnimationManager::update_tracks			()
{
	if (!need_update())
		return;
	...
	void CStalkerAnimationManager::play_script_impl			()
{
...
#ifdef USE_HEAD_BONE_PART_FAKE
	void CStalkerAnimationManager::play_global_impl			(const MotionID &animation, bool const &animation_movement_controller)
{
	torso().reset			();
	legs().reset			();
...
bool CStalkerAnimationManager::play_global					()
{
	...
	void CStalkerAnimationManager::play_head				()
{
	head().animation		(assign_head_animation());
	head().play				(m_skeleton_animated,head_play_callback,false,false);
}

	void CStalkerAnimationManager::play_torso				()
{
