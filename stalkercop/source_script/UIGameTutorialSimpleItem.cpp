...
void CUISequenceSimpleItem::Load(CUIXml* xml, int idx)
{
	...
	m_flags.set										(etiNeedPauseSound, 0==BearString::CompareWithoutCase(str, "on"));
	...
	m_flags.set						(etiGrabInput, (0==BearString::CompareWithoutCase(str_grab_input, "on")||0==BearString::CompareWithoutCase(str_grab_input, "1")) );
...
void CUISequenceSimpleItem::Start()
{
	...
	if (g_pGameLevel)
	{
		...
		if (     !BearString::CompareWithoutCase( m_pda_section, "pda_tasks"       ) ) {ui_game_sp->GetPdaMenu().SetActiveSubdialog("eptTasks");		bShowPda = true;	}
		else if( !BearString::CompareWithoutCase( m_pda_section, "pda_ranking"     ) ) {ui_game_sp->GetPdaMenu().SetActiveSubdialog("eptRanking");	bShowPda = true;	}
		else if( !BearString::CompareWithoutCase( m_pda_section, "pda_logs"        ) ) {ui_game_sp->GetPdaMenu().SetActiveSubdialog("eptLogs");		bShowPda = true;	}
