...
public:
    ...
    {
        T1* CV			= smart_cast<T1*>(values.front()); DEBUGFATALERROR1(CV);
    ...
    IC bool 			ApplyValue		(const T2& val)
    {
        ...
        m_Flags.set		(flMixed,FALSE);
    	for (PropValueIt it=values.begin(); values.end() != it; ++it){
        	T1* CV		= smart_cast<T1*>(*it); DEBUGFATALERROR1(CV);
    ...
    IC PropValue*		GetFrontValue	(){DEBUGFATALERROR1(!values.empty()); return values.front(); };
...
public:
    ...
    void*				m_FillParam;
// utils
    void				AppendChooseItem	(LPCSTR name, LPCSTR hint){DEBUGFATALERROR1(m_Items); m_Items->push_back(SChooseItem(name,hint));}
public:
