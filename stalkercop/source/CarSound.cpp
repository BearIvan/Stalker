...
void CCar::SCarSound::Init()
{
	...
	if (ini->section_exist("car_sound") && ini->line_exist("car_sound","snd_volume"))
	{
		...
		engine_start_delay=XrMath::iFloor((snd_engine_start._handle() ? XrMath::iFloor(snd_engine_start.get_length_sec()*1000.0f) : 1.f)*fengine_start_delay);
...
{
VERIFY(!physics_world()->Processing());
float		scale							= 0.5f+0.5f*pcar->m_current_rpm/pcar->m_torque_rpm; XrMath::clamp(scale,0.5f,1.25f);
