...
u32 GameEventQueue::EraseEvents(event_predicate to_del)
{
	...
	eq_iterator need_to_erase = std::find_if(ready.begin(), ready.end(), [&to_del](GameEvent* item)->bool {return to_del.call(item); });
...
//			Msg ("GameEventQueue::EraseEvents - ready %d, unused %d", ready.size(), unused.size());
		...
		++ret_val;
		need_to_erase = std::find_if(ready.begin(), ready.end(), [&to_del](GameEvent* item)->bool {return to_del.call(item); });
	}
	cs.Leave();
	return ret_val;
}
