...
#ifdef DEBUG
	if(debug_output().ph_dbg_draw_mask1().test(ph_m1_DbgTrackObject)&&(!!pObject->cName())&&BearCore::BearString::CompareWithoutCase(debug_output().PH_DBG_ObjectTrackName(),*pObject->cName())==0)
	{
...
void CPHMovementControl::Calculate(const xr_vector<DetailPathManager::STravelPathPoint>& in_path,float speed,  u32& travel_point,  float& precision  )
{
	...
	if(	debug_output().ph_dbg_draw_mask1().test(ph_m1_DbgTrackObject)&&
		( !!pObject->cName() ) && BearCore::BearString::CompareWithoutCase(debug_output().PH_DBG_ObjectTrackName(), *pObject->cName() )==0)
	{
