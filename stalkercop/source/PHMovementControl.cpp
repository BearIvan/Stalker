...
#ifdef DEBUG
	if(debug_output().ph_dbg_draw_mask1().test(ph_m1_DbgTrackObject)&&(!!pObject->cName())&&BearString::CompareWithoutCase(debug_output().PH_DBG_ObjectTrackName(),*pObject->cName())==0)
	{
									  ...
									  Fvector &											out_deviation)
{
	...
	if ( angle_between_vectors(self_vel, self_to_enemy) > XrMath::deg2rad(90.f) )
		return					false;
...
void CPHMovementControl::Calculate(const xr_vector<DetailPathManager::STravelPathPoint>& in_path,float speed,  u32& travel_point,  float& precision  )
{
	...
	if(	debug_output().ph_dbg_draw_mask1().test(ph_m1_DbgTrackObject)&&
		( !!pObject->cName() ) && BearString::CompareWithoutCase(debug_output().PH_DBG_ObjectTrackName(), *pObject->cName() )==0)
	{
...
#ifdef DEBUG
	if(ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject)&&(!!pObject->cName())&&BearString::CompareWithoutCase(PH_DBG_ObjectTrackName(),*pObject->cName())==0)
	{
