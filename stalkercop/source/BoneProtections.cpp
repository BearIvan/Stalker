...
void SBoneProtections::reload(const shared_str& bone_sect, IKinematics* kinematics)
{
	...
	for (CInifile::SectCIt i=protections.Data.begin(); protections.Data.end() != i; ++i) 
	{
		...
		BP.koeff			= (float)atof( XrTrims::GetItem( i->second.c_str(), 0, buffer) );
		BP.armor			= (float)atof( XrTrims::GetItem( i->second.c_str(), 1, buffer) );
		BP.BonePassBullet	= (BOOL) (atoi( XrTrims::GetItem(i->second.c_str(), 2, buffer) )>0.5f);
