...
#endif
			DEBUGFATALERROR1				(xr_strlen(m_sCharacterProfile));
		}
		else
			if(m_wVersion > 95)
...
void CSE_ALifeTraderAbstract::set_specific_character	(shared_str new_spec_char)
{
	...
	{
		CSE_Visual* visual = smart_cast<CSE_Visual*>(base()); DEBUGFATALERROR1(visual);
...
{
	return						(!strstr(GetCommandLine(),"-designer"));
}
#endif
...
void CSE_ALifeZoneVisual::FillProps(LPCSTR pref, PropItemVec& values)
{
	inherited1::FillProps		(pref, values);
	inherited2::FillProps		(pref, values);
	ISE_Abstract* abstract		= smart_cast<ISE_Abstract*>(this); DEBUGFATALERROR1(abstract);
...
{
	DEBUGFATALERROR1						(m_ef_weapon_type != u32(-1));
	return						(m_ef_weapon_type);
}

u32	 CSE_ALifeCreatureAbstract::ef_detector_type() const
{
	DEBUGFATALERROR1						(m_ef_detector_type != u32(-1));
...
{
	DEBUGFATALERROR1						(!m_game_death_time);
...
{
	return						(CSE_ALifeSchedulable::need_update(object) && (get_health() > XrMath::EPS_L));
}
#ifdef XRGAME_EXPORTS
...
{
	return						(!smart_cast<CSE_ALifeGroupAbstract*>(this) && (get_health() <= XrMath::EPS_L));
}
...
CSE_Abstract *CSE_ALifeOnlineOfflineGroup::init				()
{
	...
	DEBUGFATALERROR1						(m_members.empty());
...
extern void ErrorLog(LPCSTR caMessage);
#ifdef XRGAME_EXPORTS
