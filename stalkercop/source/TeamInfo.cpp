...
u32 CTeamInfo::GetTeam1_color(){
	...
	team1_color = XrColor::XrColor::color_argb(155,atoi(XrTrims::GetItem(tm_col, 0, _buff)),atoi(XrTrims::GetItem(tm_col, 1, _buff)),atoi(XrTrims::GetItem(tm_col, 2, _buff)));
...
u32 CTeamInfo::GetTeam2_color(){
	...
	team2_color = XrColor::XrColor::color_argb(155,atoi(XrTrims::GetItem(tm_col, 0, _buff)),atoi(XrTrims::GetItem(tm_col, 1, _buff)),atoi(XrTrims::GetItem(tm_col, 2, _buff)));
...
{
	string32 tmp;
		BearString::Printf(tmp, TEXT("%d"), team);
...
LPCSTR	CTeamInfo::GetTeam_color_tag(int team)
{
	...
	BearString::Printf(tmp, TEXT("%d"), team);
	R_ASSERT2( team == 1 || team == 2 || team == 3, tmp );
//	if (flags.test(flTeam1_col_t)) return *team1_color_add;
//	if (flags.test(flTeam2_col_t)) return *team2_color_add;
	if (team == 3)
	{
		team = 2;
	}
