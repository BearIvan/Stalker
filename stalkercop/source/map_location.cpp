	...
	else if ( Level().name() == map->MapName() && GetSpotPointer(sp) )
	{
		...
		if ( res )
		{
			...
			if(!bDone&&bbb)
			{
				...
				xr_vector<CLevelChanger*>::iterator lit1,lit_e;
				lit_e							= g_lchangers.end();
				for(lit1=g_lchangers.begin();lit1!=lit_e; ++lit1){
			...
			else
			{
				...
				{
					if(*ai().game_graph().header().level(ai().game_graph().vertex(*it1)->level_id()).name()==Level().name())
						break;
				}
				if(it1!=it_e1)
				{
					Fvector p = ai().game_graph().vertex(*it1)->level_point();
...
void CMapLocation::UpdateSpotPointer(CUICustomMap* map, CMapSpotPointer* sp )
{
	...
	if( map->GetPointerTo(m_position_on_map, sp->GetWidth()/2, pointer_pos, heading) )
	{
		...
		if (IsGameTypeSingle())
		{
			...
			sp->SetTextureColor( XrColor::subst_alpha(clr,a));
		}
	}
}
