...
void CLevelGraph::draw_stalkers		(const int &vertex_id)
{
	...
	if (show_text) {
		...
		OBJECT_REGISTRY::_const_iterator			I = objects.objects().begin();
		OBJECT_REGISTRY::_const_iterator			E = objects.objects().end();
		for (; I != E; ++I) {
...
//			font.OutNext		("%s",stalker->name_replace());
			...
			Fvector						position1;
			if (psAI_Flags.test(aiDrawGameGraphRealPos))
				position1 = graph.vertex(stalker->m_tGraphID)->level_point();
			else
				position1 = convert_position(graph.vertex(stalker->m_tGraphID)->game_point());
			
			render.draw_aabb	(position1,radius,radius,radius,color);
			first_time			= false;
			continue;
		}
	}
...
//			font.OutNext		("%s",monster->name_replace());
			...
			Fvector						position1;
			if (psAI_Flags.test(aiDrawGameGraphRealPos))
				position1 = graph.vertex(monster->m_tGraphID)->level_point();
			else
				position1 = convert_position(graph.vertex(monster->m_tGraphID)->game_point());
