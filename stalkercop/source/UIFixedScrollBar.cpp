...
void CUIFixedScrollBar::InitScrollBar(Fvector2 pos, bool horizontal, LPCSTR profile)
{
	...
	if(m_bIsHorizontal)
	{
		...
		m_ScrollWorkArea = XrMath::max(0,XrMath::iFloor(GetWidth()-2*height));
	}
	else
	{
		...
		m_ScrollWorkArea = XrMath::max(0,XrMath::iFloor(GetHeight()-2*width_v));
	}	
	UpdateScrollBar();
}
void CUIFixedScrollBar::UpdateScrollBar()
{
	if(IsShown())
	{
		...
		if(IsRelevant())
		{
			...
			{
				// set width
				XrMath::clamp(box_sz,XrMath::min(GetHeight(),GetWidth() - m_IncButton->GetWidth() - m_DecButton->GetWidth()),GetWidth() - m_IncButton->GetWidth() - m_DecButton->GetWidth()-2*m_ScrollBoxOffset.x);
				m_ScrollBox->SetWidth(box_sz);
				// set pos
				int pos	= PosViewFromScroll(XrMath::iFloor(box_sz),XrMath::iFloor(GetHeight()));
			...
			{
				// set height
				XrMath::clamp(box_sz,XrMath::min(GetWidth(),GetHeight()-m_IncButton->GetHeight() - m_DecButton->GetHeight()),GetHeight()-m_IncButton->GetHeight() - m_DecButton->GetHeight()-2*m_ScrollBoxOffset.y);
				m_ScrollBox->SetHeight(box_sz);
				// set pos
				int pos	= PosViewFromScroll(XrMath::iFloor(box_sz),XrMath::iFloor(GetWidth()));
	...
	if(IsRelevant())
	{
		m_ScrollBox->SetTextureColor(XrColor::color_rgba(255,255,255,255));
	}
	else
	{
		m_ScrollBox->SetTextureColor(XrColor::color_rgba(255,255,255,0));
	}
	ClampByViewRect();
}
...
bool CUIFixedScrollBar::OnMouseDownEx()
{
	...
	if(dec_rect.in(cursor_pos1) && (m_mouse_state != 2))
	{
		TryScrollDec();
		m_mouse_state = 1;
		return true;
	}	
	if(inc_rect.in(cursor_pos1) && (m_mouse_state != 1))
	{
		TryScrollInc();
		m_mouse_state = 2;
		return true;
	...
	if(dec2_rect.in(cursor_pos1) && (m_mouse_state != 2))
	{
		TryScrollDec(false);
//		m_mouse_state = 1;
		return true;
	}
	if(inc2_rect.in(cursor_pos1) && (m_mouse_state != 1))
	{
		TryScrollInc(false);
//		m_mouse_state = 2;
		return true;
...
void CUIFixedScrollBar::SetPosScrollFromView(float view_pos, float view_size, float view_offs)
{
	...
	m_iScrollPos = work_size?XrMath::iFloor(((pos/work_size)*(scroll_size) + m_iMinPos)):0; 
	XrMath::clamp(m_iScrollPos,m_iMinPos,m_iMaxPos-m_iPageSize+1);
	UpdateScrollBar();
}