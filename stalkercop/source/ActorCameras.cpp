...
#include "physics/ielevatorstate.h"
			...
			//if (tri_count)		
			{
				...
				if ( test_point( pt, mat, ext, this  ) )
				{
					...
					if (!XrMath::fis_zero(r_torso.roll))
						da		*= r_torso.roll/XrMath::abs(r_torso.roll); float angle = 0.f;
					for (;XrMath::abs(angle)<XrMath::abs(alpha); angle+=da)
					{
						Fvector				pt1;
						calc_gl_point( pt1, xform, radius, angle );
						if (test_point( pt1, mat,ext, this )) 
...
void CActor::cam_Update(float dt, float fFOV)
{
	...
	if( GamePersistent().GameType() != eGameIDSingle && ik_cam_shift && character_physics_support() && character_physics_support()->ik_controller() )
	{
		...
		if(XrMath::abs(y_shift-current_ik_cam_shift)>ik_cam_shift_tolerance)
		{
		...
		if(XrMath::abs(y_shift)<ik_cam_shift_tolerance/2.f)
			cam_smooth_k = 1.f - ik_cam_shift_speed * 1.f/0.01f * dt;
		XrMath::clamp( cam_smooth_k, 0.f, 1.f );
