...
void CWeaponStatMgun::Load(LPCSTR section)
{
	...
	camMaxAngle			= XrMath::abs( XrMath::deg2rad				(camMaxAngle) );
	camRelaxSpeed		= pSettings->r_float		(section,"cam_relax_speed"	); 
	camRelaxSpeed		= XrMath::abs( XrMath::deg2rad				(camRelaxSpeed) );

	VERIFY( !XrMath::fis_zero(camMaxAngle) );
	VERIFY( !XrMath::fis_zero(camRelaxSpeed) );
}
