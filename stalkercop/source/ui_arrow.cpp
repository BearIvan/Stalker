...
void UI_Arrow::init_from_xml( CUIXml& xml, LPCSTR path, CUIWindow* parent )
{
	...
	m_angle_end    = xml.ReadAttribFlt( path, 0, "end_angle",   XrMath::PI_MUL_2 );
	...
	if ( arrow_clockwise )
	{
		m_angle_range = -XrMath::abs( m_angle_end - m_angle_begin );
	}
	else
	{
		m_angle_range = XrMath::abs( m_angle_end - m_angle_begin );
...
{
	XrMath::clamp( new_value, 0.0f, 1.0f );
	if ( XrMath::fsimilar( m_pos, m_temp_pos ) )
	{
		m_temp_pos = m_pos + 1.05f * ( new_value - m_pos );
		XrMath::clamp( m_temp_pos, 0.0f, 1.0f );
	}
	else
	{
		...
		val	= XrMath::min( XrMath::abs(val), XrMath::abs(dif) );
		val	*= (dif > 0.0f)? +1.0f : -1.0f;
		m_pos += val;
	}
	XrMath::clamp( m_pos, 0.0f, 1.0f ); // min = 0,  max = 1
	SetPos( m_pos );
}
