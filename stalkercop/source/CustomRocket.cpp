...
#include "physics/tri-colliderknoopc/dctriangle.h"
...
void CCustomRocket::create_physic_shell	()
{
	...
	float								radius;
	CHOOSE_MAX(
		obb.m_halfsize.x,ax.set(obb.m_rotate.i) ; ax.mul(obb.m_halfsize.x); radius=XrMath::min(obb.m_halfsize.y,obb.m_halfsize.z) ;obb.m_halfsize.y/=2.f;obb.m_halfsize.z/=2.f,
		obb.m_halfsize.y,ax.set(obb.m_rotate.j) ; ax.mul(obb.m_halfsize.y); radius=XrMath::min(obb.m_halfsize.x,obb.m_halfsize.z) ;obb.m_halfsize.x/=2.f;obb.m_halfsize.z/=2.f,
		obb.m_halfsize.z,ax.set(obb.m_rotate.k) ; ax.mul(obb.m_halfsize.z); radius=XrMath::min(obb.m_halfsize.y,obb.m_halfsize.x) ;obb.m_halfsize.y/=2.f;obb.m_halfsize.x/=2.f
		)
		//radius*=1.4142f;
		...
		if(l_this->m_pOwner) 
		{
			...
			if(!l_pUD1||!l_pUD2) 
			{
				...
				if(l_pUD->pushing_neg) 
				{
					...
					if (velocity.square_magnitude() > XrMath::EPS)	
					{	//. desync?
						velocity.normalize();
						Triangle neg_tri;
