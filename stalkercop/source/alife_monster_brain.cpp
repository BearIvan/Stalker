...
CALifeMonsterBrain::CALifeMonsterBrain		(object_type *object)
{
	...
	BearCore::BearString::Scanf							(pSettings->r_string(this->object().name(),"smart_terrain_choose_interval"),"%d:%d:%d",&hours,&minutes,&seconds);
...
{
	DEBUGFATALERROR1							(object().m_smart_terrain_id != 0xffff);
	...
	DEBUGFATALERROR1							(m_smart_terrain);
	return							(*m_smart_terrain);
}
