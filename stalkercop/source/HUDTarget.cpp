...
void CHUDTarget::CursorOnFrame ()
{
	...
	{
		PP.RQ.O			= 0; 
		PP.RQ.range		= ENV.CurrentEnv->far_plane*0.99f;
		PP.RQ.element		= -1;
...
void CHUDTarget::Render()
{
	...
	{ 
		...
		{
			CEntityAlive*	E1		= smart_cast<CEntityAlive*>	(PP.RQ.O);
			...
			if (IsGameTypeSingle())
			{
				...
				else if (E1 && E1->g_Alive() && !E1->cast_base_monster())
				{
					...
					if(our_inv_owner && others_inv_owner){
						...
						{
							CStringTable	strtbl		;
							F->SetColor	(XrColor::subst_alpha(C,u8(XrMath::iFloor(255.f*(fuzzyShowInfo-0.5f)*2.f))));
				...
				if (E && (E->GetfHealth()>0))
				{
					if (pCurEnt && GameID() == eGameIDSingle)
					{
						...
						if (fuzzyShowInfo>0.5f)
						{
							...
							F->SetColor	(XrColor::subst_alpha(C,alpha_b));
