...
void	CActor::PickupModeUpdate_COD	()
{
	...
	{
		ISpatial*		spatial1	= ISpatialResult[o_it];
		...
		CGrenade*	pGrenade	= smart_cast<CGrenade*> (spatial1->dcast_CObject        ());
		if (pGrenade && !pGrenade->Useful())						continue;

		CMissile*	pMissile	= smart_cast<CMissile*> (spatial1->dcast_CObject        ());
	...
	{
		CFrustum					frustum1;
		frustum1.CreateFromMatrix	(Device.mFullTransform,FRUSTUM_P_LRTB|FRUSTUM_P_FAR);
		if (!CanPickItem(frustum1, Device.vCameraPosition, &pNearestItem->object()))
			pNearestItem = NULL;
	}
