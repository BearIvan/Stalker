#include "stdafx.h"
#include "ai/monsters/control_movement.h"
...
#ifdef DEBUG
		if (XrMath::abs(tmp) > 1000) {
			Log				("! GetVelocity",movement_control->GetVelocity());
			Log				("! GetPathDir",movement_control->GetPathDir());
		}
#endif // DEBUG
		XrMath::clamp				(tmp, 0.0f, 15.0f);
		VERIFY2				(XrMath::abs(tmp)<1000,"movement_control->GetXZActVelInGoingDir() returns too big speed");
		return				tmp;
	}
