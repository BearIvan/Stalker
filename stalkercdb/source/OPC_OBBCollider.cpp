...
BOOL OBBCollider::InitQuery(OBBCache& cache, const OBB& box, const Matrix4x4* worldb, const Matrix4x4* worldm)
{
	...
	for(udword i=0;i<3;i++)
	{
		for(udword j=0;j<3;j++)
		{
			...
			mAR.m[i][j] = 1e-6f + XrMath::abs(mRBoxToModel.m[i][j]);
		}
	}
...
*/
	...
	float NEx = XrMath::abs(mRModelToBox.m[0][0] * be.x) + XrMath::abs(mRModelToBox.m[1][0] * be.y) + XrMath::abs(mRModelToBox.m[2][0] * be.z);
	...
	float NEy = XrMath::abs(mRModelToBox.m[0][1] * be.x) + XrMath::abs(mRModelToBox.m[1][1] * be.y) + XrMath::abs(mRModelToBox.m[2][1] * be.z);
	...
	float NEz = XrMath::abs(mRModelToBox.m[0][2] * be.x) + XrMath::abs(mRModelToBox.m[1][2] * be.y) + XrMath::abs(mRModelToBox.m[2][2] * be.z);
