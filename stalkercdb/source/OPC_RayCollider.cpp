...
BOOL RayCollider::InitQuery(const Ray& world_ray, const Matrix4x4* world, udword* faceid)
{
	...
	if(IR(mMaxDist)!=IEEE_MAX_FLOAT)
	{
		...
		// Precompute mFDir;
		mFDir.x = XrMath::abs(mData.x);
		mFDir.y = XrMath::abs(mData.y);
		mFDir.z = XrMath::abs(mData.z);
	}
	else
	{
...
//		mData.z = FR(z);
		...
		// Precompute mFDir;
		mFDir.x = XrMath::abs(mDir.x);
		mFDir.y = XrMath::abs(mDir.y);
		mFDir.z = XrMath::abs(mDir.z);
	}
