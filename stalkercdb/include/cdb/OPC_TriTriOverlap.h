...
BOOL CoplanarTriTri(const Point& n, const Point& v0, const Point& v1, const Point& v2, const Point& u0, const Point& u1, const Point& u2)
{
	...
	A[0] = XrMath::abs(((const float*)n)[0]);
	A[1] = XrMath::abs(((const float*)n)[1]);
	A[2] = XrMath::abs(((const float*)n)[2]);
	if(A[0]>A[1])
	{
		if(A[0]>A[2])
		{
...
inline_ BOOL AABBTreeCollider::TriTriOverlap(const Point& V0, const Point& V1, const Point& V2, const Point& U0, const Point& U1, const Point& U2)
{
	...
	if(XrMath::abs(du0)<LOCAL_EPSILON) du0 = 0.0f;
	if(XrMath::abs(du1)<LOCAL_EPSILON) du1 = 0.0f;
	if(XrMath::abs(du2)<LOCAL_EPSILON) du2 = 0.0f;
#endif
...
#ifdef OPC_TRITRI_EPSILON_TEST
	if(XrMath::abs(dv0)<LOCAL_EPSILON) dv0 = 0.0f;
	if(XrMath::abs(dv1)<LOCAL_EPSILON) dv1 = 0.0f;
	if(XrMath::abs(dv2)<LOCAL_EPSILON) dv2 = 0.0f;
#endif
	...
	float max=XrMath::abs(((const float*)D)[0]);
	short index=0;
	float bb=XrMath::abs(((const float*)D)[1]);
	float cc=XrMath::abs(((const float*)D)[2]);
