...
*/
		...
		//! Sets a rotation matrix around the X axis.
				void				RotX(float angle)	{ float Cos = XrMath::cos(angle), Sin = XrMath::sin(angle); Identity(); m[1][1] = m[2][2] = Cos; m[2][1] = -Sin;	m[1][2] = Sin;	}
		//! Sets a rotation matrix around the Y axis.
				void				RotY(float angle)	{ float Cos = XrMath::cos(angle), Sin = XrMath::sin(angle); Identity(); m[0][0] = m[2][2] = Cos; m[2][0] = Sin;	m[0][2] = -Sin;	}
		//! Sets a rotation matrix around the Z axis.
				void				RotZ(float angle)	{ float Cos = XrMath::cos(angle), Sin = XrMath::sin(angle); Identity(); m[0][0] = m[1][1] = Cos; m[1][0] = -Sin;	m[0][1] = Sin;	}
