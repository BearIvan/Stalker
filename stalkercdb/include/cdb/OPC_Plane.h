...
#define __ICEPLANE_H__
	...
	class ICEMATHS_API Plane
	{
		...
		inline_	Plane&	Set(float nx, float ny, float nz, float d_)						{ n.Set(nx, ny, nz); d = d_;	return *this;	}
		inline_	Plane&	Set(const Point& p, const Point& _n)								{ n = _n; d = - p | n;			return *this;	}
				Plane&	Set(const Point& p0, const Point& p1, const Point& p2);

		inline_	float	Distance(const Point& p)			const						{ return (p | n) + d;								}
		inline_	bool	Belongs(const Point& p)				const						{ return XrMath::abs(Distance(p)) < PLANE_EPSILON;		}
