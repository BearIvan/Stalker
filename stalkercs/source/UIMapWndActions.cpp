...
void CMapActionZoomControl::init_internal()
{
	...
	bool bZoom					= !XrMath::fsimilar(m_targetZoom,m_object->GlobalMap()->GetCurrentZoom(),XrMath::EPS_L);
...
{
	if(m_storage->property(1)) return true;
	bool res = !!XrMath::fsimilar(m_object->GlobalMap()->GetCurrentZoom(),m_object->GlobalMap()->GetMinZoom(),XrMath::EPS_L );
	return res;
}
