...
void game_cl_Deathmatch::SetBuyMenuItems		(PRESET_ITEMS* pItems, BOOL OnlyPreset)
{
	...
	if (pCurActor)
	{
		...
		std::for_each(add_ammo.begin(), add_ammo.end(), [&](const aditional_ammo_t::value_type  & var) {AdditionalAmmoInserter(var); }/*
			std::bind1st(
				std::mem_fun<void, game_cl_Deathmatch, >(
...
//		char* ItemName = pBuyMenu->GetWeaponNameByIndex(u8(((*pItemID)&0xff00)>>0x08), u8((*pItemID)&0x00ff));
		...
		BearString::Copy(wpnAmmos, pSettings->r_string(ItemName, "ammo_class"));
