...
void CActor::IR_OnMouseMove(int dx, int dy)
{
	...
	if (dx){
		...
		cam_Active()->Move((d<0)?kLEFT:kRIGHT, XrMath::abs(d));
	}
	if (dy){
		float d = ((psMouseInvert.test(1))?-1:1)*float(dy)*scale*3.f/4.f;
		cam_Active()->Move((d>0)?kUP:kDOWN, XrMath::abs(d));
	}
}
#include "HudItem.h"
...
void	CActor::OnPrevWeaponSlot()
{
	...
	u32 NumSlotsToCheck = sizeof(SlotsToCheck) / sizeof(u32);	u32 CurSlot = 0;;
	for (; CurSlot<NumSlotsToCheck; CurSlot++)
	{
		if (SlotsToCheck[CurSlot] == ActiveSlot) break;
	};
