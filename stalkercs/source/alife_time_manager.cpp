...
void CALifeTimeManager::init			(LPCSTR section)
{
	...
	BearString::Scanf						(pSettings->r_string(section,"start_time"),"%d:%d:%d",&hours,&minutes,&seconds);
	BearString::Scanf(pSettings->r_string(section,"start_date"),"%d.%d.%d",&days,&months,&years);
