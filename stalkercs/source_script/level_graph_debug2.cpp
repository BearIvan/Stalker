...
void CLevelGraph::draw_covers	()
{
	...
	for ( ; I != E; ++I) {
		...
		CVertex				*v1 = vertex((*I)->level_vertex_id());
		Fvector				direction1;
		float				best_value1 = -1.f;
		j = 0;
		for (u32 i1=0; i1<36; ++i1) {
			float				value1 = low_cover_in_direction(float(10*i1)/180.f*XrMath::M_PI,v1);
			direction1.setHP		(float(10*i1)/180.f*XrMath::M_PI,0);
			direction1.normalize	();
			direction1.mul		(value1*half_size);
			...
			value1				= compute_low_square(float(10*i1)/180.f*XrMath::M_PI,XrMath::M_PI/2.f,v1);
		...
		float				value1 = low_cover_in_direction(float(10*j)/180.f*XrMath::M_PI,v);
		direction.setHP		(float(10*j)/180.f*XrMath::M_PI,0);
		direction.normalize	();
		direction.mul		(value1*half_size);
		direction.add		(position);
		direction.y			= position.y;
		Level().debug_renderer().draw_line	(Fidentity,position,direction,D3DCOLOR_XRGB(0,0,0));
		}
