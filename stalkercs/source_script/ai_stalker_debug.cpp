...
void CAI_Stalker::OnRender()
{
		...
		if (ai().level_graph().valid_vertex_id(level_vertex_id)) {
			...
			// low
			{
				...
				Level().debug_renderer().draw_line(Fidentity, position, direction, D3DCOLOR_XRGB(255, 0, 0));
								...
								value = ai().level_graph().low_cover_in_direction(float(10 * j1) / 180.f*XrMath::M_PI, v);
				direction.setHP(float(10 * j1) / 180.f*XrMath::M_PI, 0);
				direction.normalize();
				direction.mul(value*half_size);
				direction.add(position);
				direction.y = position.y;
	...
	for (; J != E; ++I, ++J)
		Level().debug_renderer().draw_line(Fidentity, *I, *J, D3DCOLOR_XRGB(255, 0, 0));
#endif // #if 0
}
