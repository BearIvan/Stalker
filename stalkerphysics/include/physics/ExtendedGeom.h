...
static	void RemoveCallback(CObjectContactCallback*	&callbacks,ObjectContactCallbackFun	*c)
	{
		if(!callbacks) return;
		DEBUGFATALERROR1(c);
		DEBUGFATALERROR1(callbacks->callback);
			...
			if(c==callbacks->callback)
			{
				...
				del->next=NULL;
				xr_delete(del);
				DEBUGFATALERROR1(!callbacks||!callbacks->HasCallback(c));
			} else{
				for(CObjectContactCallback	*i=callbacks->next,*p=callbacks;i;)
				{
					
					DEBUGFATALERROR1(p->callback);
					DEBUGFATALERROR1(i->callback);
					DEBUGFATALERROR1(i);
					DEBUGFATALERROR1(p);
					if(c==i->callback)
					{
						...
						DEBUGFATALERROR1(!callbacks->HasCallback(c));
						break;
					}
					i=i->next;
					p=p->next;
					DEBUGFATALERROR1(p->next==i);
				}
			}
	}
