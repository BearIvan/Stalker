...
{
	//DEBUGFATALERROR12(b_active&&island->b_active,"no active island");
	...
	*(second_island->m_bodies_tail)=first_island->firstbody;
	...
	DEBUGFATALERROR1(!(*(first_island->m_bodies_tail)));
	DEBUGFATALERROR1(!(*(first_island->m_joints_tail)));
	DEBUGFATALERROR1(!((!(first_island->nj))&&(first_island->firstjoint)));
...
{
	DEBUGFATALERROR12(m_nj==nj&&m_nb==nb&& m_flags.is_active(),"can not remove/add during processing phase");
...
IC void				ConnectJoint(dxJoint* joint)
{
	...
	{
		m_joints_tail=(dxJoint**)(&(joint->next));
		DEBUGFATALERROR1(!firstjoint);
	}
	dWorldAddJoint(DWorld(),joint);
	DEBUGFATALERROR1(!(*(m_joints_tail)));
}
