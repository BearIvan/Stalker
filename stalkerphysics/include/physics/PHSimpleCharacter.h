...
public:
	...
	virtual		void		GetBodyPosition						(Fvector& vpos)		{ DEBUGFATALERROR1(b_exist); vpos = cast_fv(dBodyGetPosition(m_body));  }
		const Fvector		&BodyPosition						()const				{DEBUGFATALERROR1(b_exist && m_body); return cast_fv(dBodyGetPosition(m_body)); } 
