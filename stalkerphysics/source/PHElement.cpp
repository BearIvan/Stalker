...
void CPHElement::			build	(){
	...
	DEBUGFATALERROR12(dMass_valide(&m_mass),"Element has bad mass");
	if(m_geoms.empty())
	{
		Fix();
	}
	...
	{
		DEBUGFATALERROR12(m_mass.mass>0.f,"Element has bad mass");
		dBodySetMass(m_body,&m_mass);
	}

	DEBUGFATALERROR1_BOUNDARIES2(m_mass_center,phBoundaries,PhysicsRefObject(),"m_mass_center");
...
void CPHElement::SetTransform(const Fmatrix &m0, motion_history_state history_state ){
	DEBUGFATALERROR12(_valid(m0), make_string( "invalid_form_in_set_transform" ) + ( PhysicsRefObject()->dump(full_capped) ) );
	DEBUGFATALERROR12(valid_pos( m0.c ),  dbg_valide_pos_string( m0.c, PhysicsRefObject(), "invalid_form_in_set_transform" ) );
	Fvector mc;
	CPHGeometryOwner::get_mc_vs_transform(mc,m0);
	DEBUGFATALERROR1_BOUNDARIES2(mc,phBoundaries,PhysicsRefObject(),"mass	center	in set transform");
	...
	DEBUGFATALERROR12(dBodyGetPosition(m_body),"not valide safe position");
	DEBUGFATALERROR12(dBodyGetLinearVel(m_body),"not valide safe velocity");
...
void CPHElement::getQuaternion(Fquaternion& quaternion)
{
	...
	DEBUGFATALERROR1(_valid(quaternion));
}
...
void CPHElement::GetGlobalPositionDynamic(Fvector* v)
{
	...
	DEBUGFATALERROR1(_valid(*v));
}

void CPHElement::SetGlobalPositionDynamic(const Fvector& position)
{
	if(!isActive()) return;
	DEBUGFATALERROR1(_valid(position));
	DEBUGFATALERROR1_BOUNDARIES2(position,phBoundaries,PhysicsRefObject(),"SetGlobalPosition argument ");
...
void CPHElement::TransformPosition(const Fmatrix &form, motion_history_state history_state  )
{
	...
	DEBUGFATALERROR1_BOUNDARIES2(new_bm.c,phBoundaries,PhysicsRefObject(),"TransformPosition dest pos");
...
void CPHElement::Activate(const Fmatrix &transform,const Fvector& lin_vel,const Fvector& ang_vel,bool disable){
	...
	DEBUGFATALERROR1(dBodyStateValide(m_body));
...
void CPHElement::Activate(const Fmatrix& start_from,bool disable){
	DEBUGFATALERROR1(_valid(start_from));
	Fmatrix globe;
...
void CPHElement::Update(){
	...
	DEBUGFATALERROR12(_valid(mXFORM),"invalid position in update");
}
...
void CPHElement::PhTune(dReal step)
{
	...
	DEBUGFATALERROR1_BOUNDARIES2(cast_fv(dBodyGetPosition(m_body)),phBoundaries,PhysicsRefObject(),"PhTune body position");
}
void CPHElement::PhDataUpdate(dReal step){
	...
	//DEBUGFATALERROR1_BOUNDARIES2(cast_fv(dBodyGetPosition(m_body)),phBoundaries,PhysicsRefObject(),"PhDataUpdate begin, body position");
#ifdef DEBUG
	...
	///////////////scale velocity///////////////////////////////////////////////////////////////////////////////////////
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	DEBUGFATALERROR1(dV_valid(linear_velocity));
#ifdef DEBUG
	if(!dV_valid(angular_velocity))
	{
		...
		DEBUGFATALERROR12(0,"bad angular velocity");
	}
#endif
	DEBUGFATALERROR1(!XrMath::fis_zero(m_l_scale));
	DEBUGFATALERROR1(!XrMath::fis_zero(m_w_scale));
	dBodySetLinearVel(
		m_body,
	...
	if(linear_velocity_mag>m_l_limit)
	{
		...
		DEBUGFATALERROR1_BOUNDARIES2(cast_fv(dBodyGetPosition(m_body)),phBoundaries,PhysicsRefObject(),"PhDataUpdate end, body position");
	...
	DEBUGFATALERROR1(dV_valid(position));
	/////////////////limit & secure angular vel///////////////////////////////////////////////////////////////////////////////
	DEBUGFATALERROR1(dV_valid(angular_velocity));
	...
	DEBUGFATALERROR1(dQ_valid(dBodyGetQuaternion(m_body)));
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	DEBUGFATALERROR1(dBodyStateValide(m_body));
	DEBUGFATALERROR12(dV_valid(dBodyGetPosition(m_body)),"invalid body position");
	DEBUGFATALERROR12(dV_valid(dBodyGetQuaternion(m_body)),"invalid body rotation");
/*
...
{
	DEBUGFATALERROR1(_valid(pos)&&_valid(dir)&&_valid(val));
	if(!isActive()||m_flags.test(flFixed)) return;
	if( !dBodyIsEnabled(m_body)) dBodyEnable(m_body);
	/////////////////////////////////////////////////////////////////////////
#ifdef DEBUG
...
void CPHElement::StataticRootBonesCallBack(CBoneInstance* B)
{
	...
	Fmatrix parent;
	DEBUGFATALERROR12( isActive(),"the element is not active");
	DEBUGFATALERROR1(_valid(m_shell->mXFORM));
	//DEBUGFATALERROR12(XrMath::fsimilar(DET(B->mTransform),1.f,DET_CHECK_EPS),"Bones callback resive 0 matrix");
	DEBUGFATALERROR1_RMATRIX(B->mTransform);
	DEBUGFATALERROR1(valid_pos(B->mTransform.c,phBoundaries));
	if(m_flags.test(flActivating))
	{
		//if(!dBodyIsEnabled(m_body))
		//	dBodyEnable(m_body);
		DEBUGFATALERROR1(!ph_world->Processing());
		DEBUGFATALERROR1(_valid(B->mTransform));
		DEBUGFATALERROR1(!m_shell->dSpace()->lock_count);
	...
	DEBUGFATALERROR1_RMATRIX(B->mTransform);
	DEBUGFATALERROR1(valid_pos(B->mTransform.c,phBoundaries));
	DEBUGFATALERROR12(_valid(B->mTransform),"Bones callback returns bad matrix");
	//else
	//{
...
{
	DEBUGFATALERROR1( range > XrMath::EPS_S );
...
//
	...
	DEBUGFATALERROR1( _valid( lv ) );
	DEBUGFATALERROR1( _valid( aw ) );
...
{
	DEBUGFATALERROR12( !ph_world->Processing(), PhysicsRefObject()->ObjectNameSect() );
	DEBUGFATALERROR1( _valid( BoneTransform ) );
	DEBUGFATALERROR1( !m_shell->dSpace()->lock_count );
...
void CPHElement::BonesCallBack( CBoneInstance* B )
{
	...
	DEBUGFATALERROR1 ( isActive() );
	DEBUGFATALERROR1( _valid( m_shell->mXFORM ) );
	//DEBUGFATALERROR12(XrMath::fsimilar(DET(B->mTransform),1.f,DET_CHECK_EPS),"Bones callback receive 0 matrix");
	DEBUGFATALERROR1_RMATRIX( B->mTransform );
	DEBUGFATALERROR1_BOUNDARIES2( B->mTransform.c, phBoundaries, PhysicsRefObject(), "BonesCallBack incoming bone position" );
...
void CPHElement::set_LinearVel			  (const Fvector& velocity)
{
	...
	if(!isActive()||m_flags.test(flFixed)) return;
	DEBUGFATALERROR12(_valid(velocity),"not valid arqument velocity");
...
{
	DEBUGFATALERROR1(_valid(velocity));
	if(!isActive()||m_flags.test(flFixed)) return;
...
{
	DEBUGFATALERROR1(_valid(x)&&_valid(y)&&_valid(z));
...
void CPHElement::calculate_it_data_use_density(const Fvector& mc,float density)
{
	...
	DEBUGFATALERROR12(dMass_valide(&m_mass),"non valide mass obtained!");
}
...
{
	DEBUGFATALERROR1(_valid(shift_pivot)&&_valid(density));
...
{
	DEBUGFATALERROR12(_valid(q)&&_valid(pos),"cv2bone_Xfrom receive wrong data");
	xform.rotation(q);
	xform.c.set(pos);
	//xform.mulB(m_inverse_local_transform);
	MulB43InverceLocalForm(xform);
	DEBUGFATALERROR12(_valid(xform),"cv2bone_Xfrom returns wrong data");
}
void CPHElement::cv2obj_Xfrom(const Fquaternion& q,const Fvector& pos, Fmatrix& xform)
{
	...
	DEBUGFATALERROR12(_valid(xform),"cv2obj_Xfrom returns wrong data");
}
...
void CPHElement::CutVelocity(float l_limit,float a_limit)
{
	...
	if(!isActive())return;
	DEBUGFATALERROR1(_valid(l_limit)&&_valid(a_limit));
