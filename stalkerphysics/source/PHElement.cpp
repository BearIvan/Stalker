...
#endif
	DEBUGFATALERROR1(!fis_zero(m_l_scale));
	DEBUGFATALERROR1(!fis_zero(m_w_scale));
	dBodySetLinearVel(
		m_body,
...
void CPHElement::StataticRootBonesCallBack(CBoneInstance* B)
{
	...
	//DEBUGFATALERROR12(fsimilar(DET(B->mTransform),1.f,DET_CHECK_EPS),"Bones callback resive 0 matrix");
...
void CPHElement::BonesCallBack( CBoneInstance* B )
{
	...
	DEBUGFATALERROR1 ( isActive() );
	DEBUGFATALERROR1( _valid( m_shell->mXFORM ) );
	//DEBUGFATALERROR12(fsimilar(DET(B->mTransform),1.f,DET_CHECK_EPS),"Bones callback receive 0 matrix");
	DEBUGFATALERROR1_RMATRIX( B->mTransform );
