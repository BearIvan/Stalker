...
#include "engine/xr_object_list.h"
...
void CPHSimpleCharacter::PhDataUpdate(dReal /**step/**/){
	...
	DEBUGFATALERROR12(dBodyStateValide(m_body),"WRONG BODYSTATE IN PhDataUpdate");
	if(PhOutOfBoundaries(cast_fv(dBodyGetPosition(m_body))))Disable();
	DEBUGFATALERROR1_BOUNDARIES(cast_fv(dBodyGetPosition(m_body)),phBoundaries,PhysicsRefObject());
...
{
	Fvector AABB1,AABB_forbid,center,center_forbid,accel_add,accel;
	
	AABB1.x=m_radius;
	AABB1.y=m_radius;
	AABB1.z=m_radius;

	AABB_forbid.set(AABB1);
	//AABB_forbid.x*=0.7f;
	//AABB_forbid.z*=0.7f;
	...
	for (CDB::RESULT* Res=R_begin; Res!=R_end; ++Res)
	{
		...
		if(m->Flags.test(SGameMtl::flPassable))continue;
		Point vertices[3]={Point((dReal*)&Res->verts[0]),Point((dReal*)&Res->verts[1]),Point((dReal*)&Res->verts[2])};
		if(__aabb_tri(Point((float*)&center),Point((float*)&AABB1),vertices)){
...
void	CPHSimpleCharacter::IPosition(Fvector& pos) {
	...
	DEBUGFATALERROR1_BOUNDARIES(pos,phBoundaries,PhysicsRefObject());
	return;
}
...
void CPHSimpleCharacter::GetPosition(Fvector& vpos)
{
	...
	DEBUGFATALERROR1_BOUNDARIES(vpos,phBoundaries,PhysicsRefObject());
}
...
void CPHSimpleCharacter::SafeAndLimitVelocity()
{
	...
	{	
		...
		if(b_external_impulse)
		{
			...
			//XrMath::clamp(ll_limit,0.f,max_hit_vel_limit);
			if(ll_limit>l_limit)
				l_limit=ll_limit;
		}
////////////////////////////////////////////////////////////////////////////////////////////
...
{
	DEBUGFATALERROR1(m_object_contact_callback!=callback);
	if(!b_exist) return;
...
//	CObject* object		=	smart_cast<CObject*>(m_phys_ref_object);
	//DEBUGFATALERROR1	(object)	;
	DEBUGFATALERROR1							(!XrMath::fis_zero(Q.dir.square_magnitude()));
	...
	else 
	{
		...
		DEBUGFATALERROR1(dInfinity!=sq_dist);
	}
	return contact_bone;
}
...
u16 CPHSimpleCharacter::DamageInitiatorID()const
{
		...
		if( m_collision_damage_info.m_obj_id != u16(-1) )
		{
			...
			DEBUGFATALERROR1(!obj|| smart_cast<IPhysicsShellHolder*>(obj) );
...
{
	DEBUGFATALERROR1(m_phys_ref_object);
	...
	DEBUGFATALERROR1(initiator_id!=u16(-1));
...
{
	DEBUGFATALERROR1(ph_world);
	DEBUGFATALERROR1(ph_world->Exist());
...
void	CPHSimpleCharacter::update_last_material()
{
	...
	u16 new_material;
	DEBUGFATALERROR1(!PhysicsRefObject() || smart_cast<CObject*>( PhysicsRefObject() ) );
