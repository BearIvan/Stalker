...
void CPHCapture::PhTune(dReal /**step/**/)
{
	...
	//	return;			//. hack
	//}
	DEBUGFATALERROR1(m_character && m_character->b_exist);
	DEBUGFATALERROR1(m_taget_object);
	DEBUGFATALERROR1(m_taget_object->ObjectPPhysicsShell());
	DEBUGFATALERROR1(m_taget_object->ObjectPPhysicsShell()->isFullActive());
	DEBUGFATALERROR1( m_taget_element );
	DEBUGFATALERROR1( m_taget_element->isFullActive());
	...
	case cstCaptured: 
		{
				...
				else 
				{
					...
					DEBUGFATALERROR1(!m_island.IsActive());
				}
		}
		break;
	case cstReleased: ;
...
void CPHCapture::PullingUpdate()
{
	...
	if(dist<m_capture_distance)
	{
		...
		DEBUGFATALERROR1( smart_cast<CPHElement*>(m_taget_element) );
...
{
	DEBUGFATALERROR1( s );
	DEBUGFATALERROR1( s->get_ElementByStoreOrder(0) );
	DEBUGFATALERROR1( s->get_ElementByStoreOrder(0)->PhysicsRefObject() );
