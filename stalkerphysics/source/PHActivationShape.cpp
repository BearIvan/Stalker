...
#ifdef DEBUG
#define	CHECK_POS(pos,msg,br)			if (!valid_pos(pos,phBoundaries)){Msg("pos:%f,%f,%f",pos.x,pos.y,pos.z);Msg(msg);DEBUGFATALERROR1(!br);}
...
CPHActivationShape::~CPHActivationShape()
{
	DEBUGFATALERROR1(!m_body&&!m_geom);
}
...
{
	DEBUGFATALERROR1(m_geom&&m_body)				;
	spatial_unregister		()			;
...
bool	CPHActivationShape::	Activate							(const Fvector need_size,u16 steps,float max_displacement,float max_rotation,bool	un_freeze_later/*	=false*/)										
{
	...
	float	limit_l_vel=XrMath::max(XrMath::max(need_size.x,need_size.y),need_size.z)/fnum_it*fnum_steps_r/fixed_step;
