...
void CPHInterpolation::UpdatePositions(){
	...
	///DEBUGFATALERROR12(dV_valid(dBodyGetPosition(m_body)),"invalid body position in update interpolation");
	qPositions.push_back(*((Fvector*) dBodyGetPosition(m_body)));
}

void CPHInterpolation::UpdateRotations(){
	DEBUGFATALERROR1(m_body);
	//DEBUGFATALERROR12(dM_valid(dBodyGetRotation(m_body)),"invalid body rotation in update interpolation");
	const dReal* dQ=dBodyGetQuaternion(m_body);
	Fquaternion fQ;
	fQ.set(-dQ[0],dQ[1],dQ[2],dQ[3]);
...
{
	DEBUGFATALERROR12(dBodyStateValide(m_body),"Invalid body state");
	qPositions.fill_in(*((Fvector*) dBodyGetPosition(m_body)));
}
