...
{
	DEBUGFATALERROR1(smart_cast<CPHElement *>(ee));
...
void CPHJoint::CreateSlider()
{
	...
	DEBUGFATALERROR1(first);
	first->GetGlobalTransformDynamic(&first_matrix);
	dBodyID body1=body_for_joint(first);
	DEBUGFATALERROR1(second);
	second->GetGlobalTransformDynamic(&second_matrix);
...
{
	DEBUGFATALERROR1( axis_num>=0 );
	DEBUGFATALERROR1( axis_num<=2 );
...
void CPHJoint::SetLoLimitDynamic(int axis_num, float limit )
{
	...
	switch(eType){
						...
						case hinge2:
								DEBUGFATALERROR1( axis_num == 0 );
								//axes[0].low = limit;
								DEBUGFATALERROR1( m_joint );
								dJointSetHinge2Param( m_joint,dParamLoStop ,limit );
								break;
						case slider:
							DEBUGFATALERROR1( axis_num <= 1 );
							switch(axis_num)
									{
...
float	CPHJoint::GetAxisAngleRate( int axis_num )
{
	...
	switch(eType){
						...
						case hinge:					DEBUGFATALERROR1(m_joint);
													ret= dJointGetHingeAngleRate(m_joint);
													break;
						case full_control:			DEBUGFATALERROR1(m_joint1);
													ret= dJointGetAMotorAngleRate(m_joint1,axis_num);
													break;
						case slider:				
													switch (axis_num)
													{
														case 0:		DEBUGFATALERROR1(m_joint);
																	ret= dJointGetSliderPositionRate(m_joint);
																	break;
														case 1:		DEBUGFATALERROR1(m_joint1);
...
void CPHJoint::SetJointFudgefactorActive(float factor)
{
	...
	switch(eType)
	{
		case hinge2:		DEBUGFATALERROR1(m_joint);
		...
		case slider:		DEBUGFATALERROR1(m_joint);
