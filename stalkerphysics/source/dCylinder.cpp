		...
		dContactGeom *contact, int skip)
{
  ...
  DEBUGFATALERROR1 (dGeomGetClass(o2) == dSphereClass);
...
int dCollideCylPlane 
  ...
  DEBUGFATALERROR1 (skip >= (int)sizeof(dContactGeom));
  DEBUGFATALERROR1 (dGeomGetClass(o1) == dCylinderClassUser);
  DEBUGFATALERROR1 (dGeomGetClass(o2) == dPlaneClass);
  contact->g1 = const_cast<dxGeom*> (o1);
  contact->g2 = const_cast<dxGeom*> (o2);
...
int dCollideCylRay(dxGeom *o1, dxGeom *o2, int flags,
				   dContactGeom *contact, int skip) {
					   DEBUGFATALERROR1 (skip >= (int)sizeof(dContactGeom));
					   DEBUGFATALERROR1 (dGeomGetClass(o1) == dCylinderClassUser);
					   DEBUGFATALERROR1 (dGeomGetClass(o2) == dRayClass);
					   contact->g1 = const_cast<dxGeom*> (o1);
					   contact->g2 = const_cast<dxGeom*> (o2);
					   dReal radius;
					   dReal lz;
...
{
  DEBUGFATALERROR12 (g && dGeomGetClass(g) == dCylinderClassUser ,"argument not a cylinder");
  DEBUGFATALERROR1 (radius > 0 && length > 0);
  dxCylinder *c = (dxCylinder*) dGeomGetClassData(g);
  c->radius = radius;
  c->lz = length;
}
