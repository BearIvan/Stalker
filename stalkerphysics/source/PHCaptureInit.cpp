...
{
	DEBUGFATALERROR1( a_character );
	DEBUGFATALERROR1( a_character->PhysicsRefObject() );
	//IKinematics* p_kinematics = smart_cast<IKinematics*>( a_character->PhysicsRefObject()->ObjectVisual( ) );
	IKinematics* p_kinematics =  a_character->PhysicsRefObject()->ObjectKinematics( ) ;
	DEBUGFATALERROR1( p_kinematics );
	CInifile* ini = p_kinematics->LL_UserData();
	DEBUGFATALERROR1( ini );
	DEBUGFATALERROR1( ini->section_exist( "capture" ) );
...

{
	...
	DEBUGFATALERROR1( m_taget_object );
	DEBUGFATALERROR1( m_taget_object->ObjectPPhysicsShell() );
	DEBUGFATALERROR1( a_character );
	m_capture_bone			=get_capture_bone( a_character );
	DEBUGFATALERROR1( m_capture_bone );
...

{
	...
	DEBUGFATALERROR1( tag_bone.callback_param( ) );

	m_taget_element					=(CPhysicsElement*)tag_bone.callback_param();
	DEBUGFATALERROR1( m_taget_element );
...
{
	DEBUGFATALERROR1( m_taget_element );
	DEBUGFATALERROR1( m_character );
	DEBUGFATALERROR1( m_character->PhysicsRefObject() );
	//DEBUGFATALERROR1( m_character->PhysicsRefObject()->ObjectVisual( ) );
	DEBUGFATALERROR1( m_character->PhysicsRefObject()->ObjectKinematics( ) );
	DEBUGFATALERROR1( m_capture_bone );
...
void CPHCapture::Release()
{
	if( e_state==cstReleased || e_state== cstFree ) 
		return;
	DEBUGFATALERROR1( m_island.DActiveIsland() == &m_island );
...
void CPHCapture::Deactivate()
{
	...
	if(m_taget_element)
	{
		DEBUGFATALERROR1(dynamic_cast<CPHElement*>(m_taget_element));
