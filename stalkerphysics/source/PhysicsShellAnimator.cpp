...
{
	DEBUGFATALERROR1( ini->section_exist( section ) );
...
void	CPhysicsShellAnimator::	CreateJoints( LPCSTR controled )
{
		...
		for(u16 i = 0 ; nb > i ; ++i )
		{
			...
			DEBUGFATALERROR12( e, make_string( "shell_animation - controled bone %s has no physics collision! object: %s, model: %s", n, obj->ObjectName(), obj->ObjectNameVisual() ) );
			CreateJoint( e );
		}
}
...
void CPhysicsShellAnimator::OnFrame()
{
	...
	for (xr_vector<CPhysicsShellAnimatorBoneData>::iterator i=m_bones_data.begin();i!=m_bones_data.end();i++)
	{
		...
		B.Callback = 0;   B.Callback_overwrite = 0;
