	...
	if(start_joint!=end_joint)
	{
		...
		for(;i!=e;++i)
		{
			...
			if(joint->PFirst_element()==source_element)
			{
				...
				DEBUGFATALERROR1(smart_cast<CPHElement*>(joint->PSecond_element()));
...
///////////////////temporary for initialization set old Kinematics in new shell/////////////////
	...
	DEBUGFATALERROR12(split_elem.second.m_bone_id<64,"strange root");
	DEBUGFATALERROR1(_valid(new_shell_last->mXFORM));
	DEBUGFATALERROR1(dBodyStateValide(source_element->get_bodyConst()));
	DEBUGFATALERROR1(dBodyStateValide(split_elem.first->get_body()));
...
void CPHShellSplitterHolder::SplitElement(u16 aspl,PHSHELL_PAIR_VECTOR &out_shels)
{
	...
	for(;i!=e;++i)
	{
		...
		CPHElement * e1 = static_cast<CPHElement *> (ee);
		DEBUGFATALERROR1(dBodyStateValide(e1->get_body()));
	}
...
void CPHShellSplitterHolder::SplitProcess(PHSHELL_PAIR_VECTOR &out_shels)
{
	...
	//DEBUGFATALERROR1(dBodyStateValide(out_shels.back().first->get_ElementByStoreOrder(0)->get_body()));
	m_has_breaks=false;
}
