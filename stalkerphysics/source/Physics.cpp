...
const float 		default_w_limit									= 9.8174770f;//(XrMath::M_PI/16.f/(fixed_step=0.02f));
...
IC static int CollideIntoGroup(dGeomID o1, dGeomID o2,dJointGroupID jointGroup,CPHIsland* world,const int &MAX_CONTACTS)
{
	...
	DEBUGFATALERROR1	(o1);
	DEBUGFATALERROR1	(o2); 
	DEBUGFATALERROR1(&contacts[0].geom);
...
{
	DEBUGFATALERROR12(dJointGetType(joint)==dJointTypeContact,"not a contact!");
