						...
						)
{
  ...
  DEBUGFATALERROR1 (dGeomGetClass(o1) == dBoxClass);
...
//}
 for (u32 i1=0; i1<ret; ++i1) {
    CONTACT(contact,i1*skip)->g1 = const_cast<dxGeom*> (o2);
    CONTACT(contact,i1*skip)->g2 = const_cast<dxGeom*> (o1);
	CONTACT(contact,i1*skip)->normal[0] = norm[0];
	CONTACT(contact,i1*skip)->normal[1] = norm[1];
	CONTACT(contact,i1*skip)->normal[2] = norm[2];
	SURFACE(contact,i1*skip)->mode=T->T->material;
 }
