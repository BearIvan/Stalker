...
{
	DEBUGFATALERROR1			( m_geom_transform );
...
void CODEGeom::	clear_motion_history( bool set_unspecified )
{
	...
	Fmatrix m;
	get_xform( m );
	if( Fvector().sub( m.c, cast_fv ( dGeomGetUserData( geom() )->last_pos ) ).magnitude() > XrMath::EPS )
		Msg("! WRONG THING" );
#endif
}
...
void CODEGeom::clear_cashed_tries()
{
	...
	if(g)
	{
		DEBUGFATALERROR1(dGeomGetUserData(g));
		dGeomUserDataClearCashedTries(g);
	}
	else
	{
		DEBUGFATALERROR1(dGeomGetUserData(m_geom_transform));
...
void CODEGeom::set_material(u16 ul_material)
{
	...
	if(geom())
	{
		DEBUGFATALERROR1(dGeomGetUserData(geom()));
