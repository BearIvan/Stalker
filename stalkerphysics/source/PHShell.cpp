...
void    CPHShell::  applyImpulseTrace       (const Fvector& pos, const Fvector& dir, float val,const u16 id){
    if(!isActive()) return;
    DEBUGFATALERROR1(m_pKinematics);
...
void    CPHShell:: update_root_transforms           ()
{
    ...
    DEBUGFATALERROR1( BI_NONE != anim_root );
    DEBUGFATALERROR1( BI_NONE != phys_root );
...
void  CPHShell:: BonesCallback              ( CBoneInstance* B ){
    ...
    //}
    E->BonesCallBack(B);
    DEBUGFATALERROR12( _valid(B->mTransform), "CPHShell:: BonesCallback" );
}
    ...
    {   
        ...
        if(joint_data.type==jtRigid && root_e ) //
        {
            ...
            if(breakable)
            {
                ...
                DEBUGFATALERROR1                              (u16(-1)!=fracture.m_start_geom_num);
...
#ifdef DEBUG
    if(E->last_geom())
    DEBUGFATALERROR1(E->last_geom()->bone_id()!=u16(-1));
#endif
...
void CPHShell::SetCallbacks( )
{
    ...
    for_each_bone_id<set_bone_reference>( *PKinematics() ,set_bone_reference( *PKinematics() ) );
...
void CPHShell::InterpolateGlobalTransform(Fmatrix* m)
{
    ...
    DEBUGFATALERROR12(_valid(*m),"not valide transform");
...
void CPHShell::InterpolateGlobalPosition(Fvector* v)
{
    ...
    DEBUGFATALERROR12(_valid(*v),"not valide result position");
}
...
void CPHShell::ObjectToRootForm(const Fmatrix& form)
{
    ...
    M.invert();
    mXFORM.mul(form,M);
    DEBUGFATALERROR12(_valid(form),"not valide transform");
...
void CPHShell::PassEndElements(u16 from,u16 to,CPHShell *dest)
{
    ...
    for(ELEMENT_I i=i_from;i!=e;++i)
    {
        ...
        DEBUGFATALERROR1(_valid(dest->mXFORM));
        (*i)->SetShell(dest);
    }
...
bool CPHShell::get_ApplyByGravity()
{
    ...
    DEBUGFATALERROR1      (elements.front());
...
{   
    ...
    DEBUGFATALERROR1( PhysicsRefObject( ) );
...
{
    DEBUGFATALERROR1( PPhysicsShellAnimator() );
