...
CPhysicsShell	* create_camera_shell( IPhysicsShellHolder *actor )
{
	...
	DEBUGFATALERROR1(smart_cast<CPHElement*>(roote));
	...
	DEBUGFATALERROR1( roote );
	roote->set_local_mass_center( Fvector().set(0,0,0) );
	DEBUGFATALERROR1( roote->numberOfGeoms() );
	CODEGeom	*root_geom = roote->geometry( 0 );
	DEBUGFATALERROR1( root_geom );
...
void set_camera_collision( const Fvector &box_size, const Fmatrix &xform, CPhysicsElement *roote, CBoxGeom* box )
{
	...
	DEBUGFATALERROR1( character_collision_geom );
...
void	do_collide_and_move(const Fmatrix &xform, IPhysicsShellHolder* l_actor, CPhysicsShell	*shell, CPhysicsElement *roote )
{
	...
	cam_collided = false;
	cam_step = false;
	DEBUGFATALERROR1( l_actor );
	//DEBUGFATALERROR1( l_actor->character_physics_support() );
	//DEBUGFATALERROR1( l_actor->character_physics_support()->movement() );
